...
for %%i in (temp_java\ortools-win32-x86-64\target\*.jar*) do copy %%i export\.
for %%i in (temp_java\ortools-java\target\*.jar*) do copy %%i export\.
...
for %%i in (packages\*nupkg*) do copy %%i export\.
...
for %%i in (%TEMP_DIR%\ortools\dist\*.whl) do copy %%i export\.
)
