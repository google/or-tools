...
ifeq ("$(SYSTEM)","unix")
  ...
  ifeq ($(OS),Linux)
    ...
    ORTOOLS_LIB = $(OR_TOOLS_LIB_PRE) -lcvrptw_lib $(OR_TOOLS_LIB_PRE) -ldimacs $(OR_TOOLS_LIB_PRE) -lortools
  ...
  ifeq ($(OS),Darwin) # Assume Mac Os X
    ...
    LD_FLAGS = -lz
    ORTOOLS_LIB = -L$(OR_ROOT)lib -lcvrptw_lib -L$(OR_ROOT)lib -ldimacs -L$(OR_ROOT)lib -lortools
    ARCH = -DARCH_K8
    PORT = MacOsX64
  endif
  O = o
...
ifeq ("$(SYSTEM)","win")
  ...
  ORTOOLS_LIB = lib\\cvrptw.lib lib\\dimacs.lib lib\\ortools.lib
  O=obj
  E=.exe
  OBJ_OUT = /Fo
  EXE_OUT = /Fe
