...
def main():
  ...
  for p in all_people:
    ...
    people_meeting_copy = solver.FixedDurationIntervalVar(
        1, max_time, meeting_duration, True, name)
    people_meeting_copies[p] = people_meeting_copy

  room_meeting_copies = {}
  for r in all_rooms:
    name = 'room meeting copy %d' % r
    room_meeting_copy = solver.FixedDurationIntervalVar(
        1, max_time, meeting_duration, True, name)
  ...
  for p in all_people:
    lunch = solver.FixedDurationIntervalVar(
        lunch_start_quarter, lunch_end_quarter - lunch_duration, lunch_duration,
        False, 'lunch for %d' % p)
    calendar = [people_meeting_copies[p], lunch]
    calendar.extend(
        [t['meeting'] for t in existing_meetings if t['person'] == p])
  ...
  for r in all_rooms:
    ...
    calendar.extend(
        [t['reservation'] for t in room_reservations if t['room'] == r])
  ...
  solver.Add(
      people_count == solver.Sum([all_people_presence[p] for p in all_people]))
  # Mandatory persons.
  ...
  for p in all_people:
    solver.Add(
        all_people_presence[p] == people_meeting_copies[p].PerformedExpr())

  # Synchronize all_presences and meetings.
  for r in all_rooms:
    solver.Add(all_rooms_presence[r] == room_meeting_copies[r].PerformedExpr())

  # Map meeting_location to room_meeting presence.
  solver.Add(
      (meeting_location - 1).MapTo([all_rooms_presence[p] for p in all_rooms]))
  ...
  if collector.SolutionCount() > 0:
    ...
    print('we could schedule %d persons in room %d starting at quarter %d' %
          (collector.Value(0, people_count), collector.Value(
              0, meeting_location), collector.StartValue(0, meeting)))
