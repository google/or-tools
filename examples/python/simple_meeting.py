...
def main():
    ...
    for p in all_people:
        lunch = solver.FixedDurationIntervalVar(
            lunch_start_quarter, lunch_end_quarter - lunch_duration,
            lunch_duration, False, 'lunch for %d' % p)
    ...
    solver.Add(people_count == solver.Sum(
        [all_people_presence[p] for p in all_people]))
    # Mandatory persons.
    ...
    for r in all_rooms:
        solver.Add(
            all_rooms_presence[r] == room_meeting_copies[r].PerformedExpr())

    # Map meeting_location to room_meeting presence.
    solver.Add((meeting_location - 1).MapTo(
        [all_rooms_presence[p] for p in all_rooms]))
