...
def Solve(type):
  ...
  db = solver.phase(vars, cp.IntVarStrategy.CHOOSE_FIRST_UNBOUND, cp.IntValueStrategy.ASSIGN_MAX_VALUE)
  ls = None
  ...
  if type == 0:  # LNS
    ...
    ls_params = solver.local_search_phase_parameters(sum_var, move_one_var, db, None,
                                                     filter_manager)
    ls = solver.local_search_phase(vars, db, ls_params)
