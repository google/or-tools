...
def main():
  ...
  for j in range(num_nurses):
    for i in range(num_days):
      shifts[(j, i)] = solver.new_int_var(0, num_shifts - 1,
                                     "shifts(%i,%i)" % (j, i))
  shifts_flat = [
  ...
  for j in range(num_shifts):
    for i in range(num_days):
      nurses[(j, i)] = solver.new_int_var(0, num_nurses - 1,
                                     "shift%d day%d" % (j, i))
  ...
  db = solver.phase(shifts_flat, cp.IntVarStrategy.CHOOSE_FIRST_UNBOUND,
