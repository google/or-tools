...
def main():
    ...
    nurse_stat = [
        solver.new_int_var(0, num_days, "nurse_stat[%i]" % i) for i in range(num_nurses)
    ]
    ...
    for i in range(num_days):
        for j in shifts:
            day_stat[i, j] = solver.new_int_var(
                0, num_nurses, "day_stat[%i,%i]" % (i, j)
            )
    ...
    for i in range(num_nurses):
        ...
        b = [
            solver.add_is_equal_cst_var(x[i, j], day_shift)
            + solver.add_is_equal_cst_var(x[i, j], night_shift)
            for j in range(num_days)
        ]
