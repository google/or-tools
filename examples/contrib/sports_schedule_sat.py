# Based on sports_scheduling_sat.cc, Copyright 2010-2025 Google LLC
#
    ...
    for home in range(num_teams):
      for away in range(num_teams):
        if solver.Value(fixtures[d][home][away]):
          ...
          row = {'day': d + 1, 'game': game, 'home': home + 1, 'away': away + 1}
          vcsv.append(row)
...
def assign_matches(
  ...
  if status != cp_model.OPTIMAL and solver.WallTime() >= time_limit:
    print(
        'Please note that solver reached maximum time allowed %i.' % time_limit
    )
    print(
        'A better solution than %i might be found by adding more time using the'
        ' --timelimit command line option'
