async function rn(N = {}) {
  ...
  var u = N, B = !!globalThis.window, z = !!globalThis.WorkerGlobalScope, H = globalThis.process?.versions?.node && globalThis.process?.type != "renderer", me = !B && !H && !z, S = z && self.name?.startsWith("em-pthread");
  ...
  d(B || z || H, "Pthreads do not work in this environment yet (need Web Workers, or an alternative to them)"), d(!H, "node environment detected but not enabled at build time.  Add `node` to `-sENVIRONMENT` to enable."), d(!me, "shell environment detected but not enabled at build time.  Add `shell` to `-sENVIRONMENT` to enable.");
  var Pe;
  globalThis.WebAssembly || k("no native wasm support detected");
  var le, ee = !1, ve;
  function d(e, r) {
    e || D("Assertion failed" + (r ? ": " + r : ""));
  }
  ...
  function De() {
    if (!ee) {
      ...
      (r != 34821223 || t != 2310721022) && D(`Stack overflow! Stack cookie has been overwritten at ${fe(e)}, expected hex dwords 0x89BACDFE and 0x2135467, but received ${fe(t)} ${fe(r)}`), (f(), y)[0] != 1668509029 && D("Runtime error: The application has corrupted its heap memory area (address zero)!");
    }
  }
  function Ze(...e) {
    console.warn(...e);
  ...
  (() => {
    ...
    e[0] = 25459, (r[0] !== 115 || r[1] !== 99) && D("Runtime error: expected the system to be little-endian! (Run with -sSUPPORT_BIG_ENDIAN to bypass)");
  })();
  function Le(e) {
    Object.getOwnPropertyDescriptor(u, e) || Object.defineProperty(u, e, { configurable: !0, set() {
      D(`Attempt to set \`Module.${e}\` after it has already been processed.  This can happen, for example, when code is injected via '--post-js' rather than '--pre-js'`);
    } });
  }
  function C(e) {
  ...
  function on(e) {
    Object.getOwnPropertyDescriptor(u, e) && D(`\`Module.${e}\` was supplied but \`${e}\` not included in INCOMING_MODULE_JS_API`);
  }
  function sn(e) {
  ...
  function Pr(e) {
    S || Object.getOwnPropertyDescriptor(u, e) || Object.defineProperty(u, e, { configurable: !0, get() {
      ...
      sn(e) && (r += ". Alternatively, forcing filesystem support (-sFORCE_FILESYSTEM) can export this for you"), D(r);
    } });
  }
  function dn() {
    function e() {
      var t = 0;
      return pe && typeof Te < "u" && (t = Te()), `w:${Dr},t:${fe(t)}:`;
    }
    var r = Ze;
    Ze = (...t) => r(e(), ...t);
  }
  ...
  if (S) {
    let e = function(r) {
      try {
        ...
        } else t.target === "setimmediate" || (n === "checkMailbox" ? rr && He() : n && (k(`worker: received unknown command ${n}`), k(t)));
      } catch (a) {
  ...
  var j, G, de, Ie, W, y, Mr, Me, U, Rr, pe = !1;
  function Ue() {
    var e = Y.buffer;
    j = new Int8Array(e), de = new Int16Array(e), u.HEAPU8 = G = new Uint8Array(e), Ie = new Uint16Array(e), W = new Int32Array(e), y = new Uint32Array(e), Mr = new Float32Array(e), Me = new Float64Array(e), U = new BigInt64Array(e), Rr = new BigUint64Array(e);
  }
  function cn() {
    if (!S) {
      if (u.wasmMemory)
  ...
  function Nr() {
    if (d(!pe), pe = !0, S) return Ir();
    De(), !u.noFSInit && !i.initialized && i.init(), ne.__wasm_call_ctors(), i.ignorePermissions = !1;
  }
  function fn() {
    if (De(), !S) {
      if (u.postRun)
  ...
  async function pn(e, r) {
    try {
      ...
      k(`failed to asynchronously prepare wasm: ${a}`), Cr(e) && k(`warning: Loading from a file URI (${e}) is not supported in most browsers. See https://emscripten.org/docs/getting_started/FAQ.html#how-do-i-run-a-local-webserver-for-testing-why-does-my-program-stall-in-downloading-or-preparing`), D(a);
    }
  }
  ...
  var Ur = (e) => {
    ...
    for (; e.length > 0; )
      e.shift()(u);
  }, $r = [], Br = (e) => $r.push(e), ce = 0, Re = null, he = {}, ue = null, zr = (e) => {
  ...
  var yn = (e, r) => {
    ...
    Br(async () => {
      ...
      Hr("loading-workers"), await r, zr("loading-workers");
    });
  }, terminateAllThreads: () => {
    ...
    var r = e.pthread_ptr;
    delete T.pthreads[r], T.unusedWorkers.push(e), T.runningWorkers.splice(T.runningWorkers.indexOf(e), 1), e.pthread_ptr = 0, zt(r);
  }, threadInitTLS() {
    ...
    e.onmessage = (o) => {
      ...
      l === "checkMailbox" ? He() : l === "spawnThread" ? Vr(s) : l === "cleanupThread" ? _r(() => xr(s.thread)) : l === "loaded" ? (e.loaded = !0, r(e)) : s.target === "setimmediate" ? e.postMessage(s) : l === "callHandler" ? u[s.handler](...s.args) : l && k(`worker sent an unknown command ${l}`);
    }, e.onerror = (o) => {
    ...
    if (u.mainScriptUrlOrBlob) {
      ...
      /* @vite-ignore */
      "/assets/cp_sat_runtime-DkzQa7sN.js",
      import.meta.url
  ...
  var sr = !0, bn = (e) => T.tlsInitFunctions.push(e), ae = (e) => {
    ...
    return o;
  }, re = (e, r, t) => (d(typeof e == "number", `UTF8ToString expects a number (got ${typeof e})`), e ? ge((f(), G), e, r, t) : ""), kn = (e, r, t, n) => D(`Assertion failed: ${re(e)}, at: ` + [r ? re(r) : "unknown filename", t, n ? re(n) : "unknown function"]);
  class Tn {
    constructor(r) {
  ...
  }, we = je, I = { isAbs: (e) => e.charAt(0) === "/", splitPath: (e) => {
    var r = /^(\/?|)([\s\S]*?)((?:\.{1,2}|[^\/]+?|)(\.[^.\/]*|))(?:[\/]*)$/;
  ...
  }, basename: (e) => e && e.match(/([^\/]+|\/)\/*$/)[1], join: (...e) => I.normalize(e.join("/")), join2: (e, r) => I.normalize(e + "/" + r) }, An = () => (e) => e.set(crypto.getRandomValues(new Uint8Array(e.byteLength))), lr = (e) => {
    (lr = An())(e);
  }, Ee = { resolve: (...e) => {
...
`)), !e)
  ...
  } } }, Dn = (e, r) => (f(), G).fill(0, e, e + r), rt = (e, r) => (d(r, "alignment argument is required"), Math.ceil(e / r) * r), tt = (e) => {
    e = rt(e, 65536);
    var r = jt(65536, e);
    return r && Dn(r, e), r;
    ...
    var c = r ? Ee.resolve(I.join2(e, r)) : e, _ = On(`cp ${c}`);
    Hr(_);
    try {
      var h = t;
    ...
    r.flags = r.flags || 0, r.encoding = r.encoding || "binary", r.encoding !== "utf8" && r.encoding !== "binary" && D(`Invalid encoding type "${r.encoding}"`);
    ...
    typeof r == "string" && (r = new Uint8Array(cr(r))), ArrayBuffer.isView(r) ? i.write(n, r, 0, r.byteLength, void 0, t.canOwn) : D("Unsupported data type"), i.close(n);
    ...
    var o = I.join2(typeof e == "string" ? e : i.getPath(e), r), s = ur(n, a);
    return i.create(o, s);
    ...
    var a = I.join2(typeof e == "string" ? e : i.getPath(e), r), o = ur(!!t, !!n);
    ...
    if (globalThis.XMLHttpRequest)
      D("Lazy loading should have been performed (contents set) in createLazyFile, but it was not. Lazy loading only works in web workers. Use --embed-file or --preload-file in emcc on the main thread.");
    else
      try {
        e.contents = We(e.url);
      } catch {
    ...
    class o {
      ...
      cacheLength() {
        ...
        p.open("HEAD", t, !1), p.send(null), p.status >= 200 && p.status < 300 || p.status === 304 || D("Couldn't load " + t + ". Status: " + p.status);
        ...
        O || (L = g);
        var V = (Q, Ce) => {
          Q > Ce && D("invalid range (" + Q + ", " + Ce + ") or no bytes requested!"), Ce > g - 1 && D("only " + g + " bytes available! programmer error!");
          var $ = new XMLHttpRequest();
          return $.open("GET", t, !1), g !== L && $.setRequestHeader("Range", "bytes=" + Q + "-" + Ce), $.responseType = "arraybuffer", $.overrideMimeType && $.overrideMimeType("text/plain; charset=x-user-defined"), $.send(null), $.status >= 200 && $.status < 300 || $.status === 304 || D("Couldn't load " + t + ". Status: " + $.status), $.response !== void 0 ? new Uint8Array($.response || []) : cr($.responseText || "");
        }, Oe = this;
        Oe.setDataGetter((Q) => {
          var Ce = Q * L, $ = (Q + 1) * L - 1;
          return $ = Math.min($, g - 1), typeof Oe.chunks[Q] > "u" && (Oe.chunks[Q] = V(Ce, $)), typeof Oe.chunks[Q] > "u" && D("doXHR failed!"), Oe.chunks[Q];
    ...
    if (globalThis.XMLHttpRequest) {
      z || D("Cannot do synchronous binary XHRs outside webworkers in modern browsers. Use --embed-file or --preload-file in emcc");
    ...
    D("FS.absolutePath has been removed; use PATH_FS.resolve instead");
  }, createFolder() {
    D("FS.createFolder has been removed; use FS.mkdir instead");
  }, createLink() {
    D("FS.createLink has been removed; use FS.symlink instead");
  }, joinPath() {
    D("FS.joinPath has been removed; use PATH.join instead");
  }, mmapAlloc() {
    D("FS.mmapAlloc has been replaced by the top level function mmapAlloc");
  }, standardizePath() {
    D("FS.standardizePath has been removed; use PATH.normalize instead");
  ...
  var Un = () => D("native code called abort()"), q = (e) => {
    for (var r = ""; ; ) {
      var t = (f(), G)[e++];
      if (!t) return r;
  ...
  var _t = (e, r, t) => {
    switch (r) {
      ...
      case 2:
        return t ? (n) => (f(), de)[n >> 1] : (n) => (f(), Ie)[n >> 1];
      case 4:
  ...
  }, vt = [], te = [0, 1, , 1, null, 1, !0, 1, !1, 1], zn = (e) => {
  ...
  var Hn = { name: "emscripten::val", fromWireType: (e) => {
    var r = pt.toValue(e);
    return zn(e), r;
  }, toWireType: (e, r) => pt.toHandle(r), readValueFromPointer: fr, destructorFunction: null }, Vn = (e) => X(e, Hn), Gn = (e, r) => {
    switch (r) {
      case 4:
        return function(t) {
...
${_}`, new Function(w, _);
  ...
  var ze = (e) => {
    ...
    if (e instanceof Lr || e == "unwind")
      return ve;
    De(), e instanceof WebAssembly.RuntimeError && br() <= 0 && k("Stack overflow detected.  You can try increasing -sSTACK_SIZE (currently set to 65536)"), Fr(1, e);
  }, Jn = () => {
    if (!xe())
      try {
        if (S) {
      ...
      if (typeof n == "function") {
        ...
        e[t] = (...o) => {
          ...
          try {
            ...
            m.state !== s && !a && !l && !c && D(`import ${t} was not in ASYNCIFY_IMPORTS, but changed the state`);
          }
        };
      }
    ...
    m.currData && m.state === m.State.Unwinding && m.exportCallStack.length === 0 && (m.state = m.State.Normal, mr(), ze(Jt), typeof Fibers < "u" && Fibers.trampoline());
  }, whenDone() {
      ...
      if (m.state === m.State.Normal) {
        ...
        e((n = 0) => {
          if (d(!n || typeof n == "number" || typeof n == "boolean"), !ee && (m.handleSleepReturnValue = n, r = !0, !!t)) {
            d(!m.exportCallStack.length, "Waking up (starting to rewind) must be done from JS, without compiled code on the stack."), m.state = m.State.Rewinding, ze(() => Zt(m.currData)), typeof MainLoop < "u" && MainLoop.func && MainLoop.resume();
            var a, o = !1;
            try {
        ...
        }), t = !0, r || (m.state = m.State.Unwinding, m.currData = m.allocateData(), typeof MainLoop < "u" && MainLoop.func && MainLoop.pause(), ze(() => Xt(m.currData)));
      } else m.state === m.State.Rewinding ? (m.state = m.State.Normal, ze(Qt), Z(m.currData), m.currData = null, m.sleepCallbacks.forEach(_r)) : D(`invalid state: ${m.state}`);
  ...
  var li = (e) => {
    ...
    var n = e >> 1, a = Jr((f(), Ie), n, r / 2, t);
    if (a - n > 16 && St) return St.decode((f(), Ie).slice(n, a));
    for (var o = "", s = n; s < a; ++s) {
      var l = (f(), Ie)[s];
    ...
    wr(e, !z, 1, !B, 65536, !1), T.threadInitTLS();
  }, vr = (e) => {
    if (Atomics.waitAsync) {
      var r = Atomics.waitAsync((f(), W), e >> 2, e);
      d(r.async), r.value.then(He);
      var t = e + 128;
  ...
  }, Pi = 9007199254740992, Di = -9007199254740992, pr = (e) => e < Di || e > Pi ? NaN : Number(e);
  ...
  var Ge = [], Oi = (e, r) => {
    ...
    z || ae("Blocking on the main thread is very dangerous, see https://emscripten.org/docs/porting/pthreads.html#blocking-on-the-main-browser-thread");
  }, xi = (e, r) => k(re(e, r)), ji = () => {
    throw mr(), "unwind";
  ...
  }, Pt = () => new Error().stack.toString(), zi = () => {
    var e = Pt().split(`
`);
    ...
    for (var n = 1; n <= 4; n *= 2) {
      ...
      var o = Math.min(t, rt(Math.max(e, a), 65536)), s = Hi(o);
      if (s)
        return !0;
    }
...
`), n[0] == "Error" && n.shift(), At(n));
  ...
  na.forEach(Pr);
  var ia = [ar, qr, Zr, ot, st, lt, dt, ct, ut, ft, bt, kt, Dt, It, Mt, Rt, Nt, Ot];
  function aa() {
    on("fetchSettings");
  }
  ...
  for (const e of Object.keys(u))
    e in N || Object.defineProperty(N, e, { configurable: !0, get() {
      D(`Access to module property ('${e}') is no longer possible via the module constructor argument; Instead, use the result of the module constructor.`);
    } });
  return A;
}
...
async function va(N, A) {
  ...
  const u = M._malloc(4), B = kr(N), z = kr(A ?? null);
  let H = 0;
  try {
    H = await M.ccall(
      "solve_model",
    ...
    B && M._free(B), z && M._free(z);
  }
  const me = tn(M.HEAPU8.buffer, u);
  if (M._free(u), !H || me === 0)
    return H && M._free_buffer(H), new Uint8Array();
  const S = M.HEAPU8.slice(H, H + me);
  return M._free_buffer(H), S;
}
async function pa(N) {
  if (!M)
    throw new Error("Module not initialized.");
  ...
  const z = tn(M.HEAPU8.buffer, A);
  if (M._free(A), !B || z === 0)
    return B && M._free_buffer(B), { ok: !0, message: "" };
  const H = M.HEAPU8.slice(B, B + z);
  return M._free_buffer(B), { ok: !1, message: new TextDecoder().decode(H) };
}
...
//# sourceMappingURL=cpsat_worker-SfG63Q3p.js.map
