...
$(OBJ_DIR)/parse_dimacs_assignment.$O:$(EX_DIR)examples/parse_dimacs_assignment.cc
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/parse_dimacs_assignment.cc $(OBJ_OUT)parse_dimacs_assignment.$O
$(OBJ_DIR)/print_dimacs_assignment.$O:$(EX_DIR)examples/print_dimacs_assignment.cc
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/print_dimacs_assignment.cc $(OBJ_OUT)print_dimacs_assignment.$O
...
$(OBJ_DIR)/linear_assignment_api.$O:$(EX_DIR)examples/linear_assignment_api.cc
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/linear_assignment_api.cc $(OBJ_OUT)linear_assignment_api.$O
...
$(OBJ_DIR)/flow_api.$O:$(EX_DIR)examples/flow_api.cc
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/flow_api.cc $(OBJ_OUT)flow_api.$O

$(BIN_DIR)/flow_api$E: $(GRAPH_DEPS) $(OBJ_DIR)/flow_api.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/flow_api.$O $(GRAPH_LNK) $(LDFLAGS) $(EXEOUT)flow_api$E

$(OBJ_DIR)/dimacs_assignment.$O:$(EX_DIR)examples/dimacs_assignment.cc
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/dimacs_assignment.cc $(OBJ_OUT)dimacs_assignment.$O
...
$(OBJ_DIR)/costas_array.$O: $(EX_DIR)examples/costas_array.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/costas_array.cc $(OBJ_OUT)costas_array.$O
...
$(OBJ_DIR)/cryptarithm.$O:$(EX_DIR)examples/cryptarithm.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/cryptarithm.cc $(OBJ_OUT)cryptarithm.$O

$(BIN_DIR)/cryptarithm$E: $(CP_DEPS) $(OBJ_DIR)/cryptarithm.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/cryptarithm.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)cryptarithm$E

$(OBJ_DIR)/cvrptw.$O: $(EX_DIR)examples/cvrptw.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/cvrptw.cc $(OBJ_OUT)cvrptw.$O

$(BIN_DIR)/cvrptw$E: $(ROUTING_DEPS) $(OBJ_DIR)/cvrptw.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/cvrptw.$O $(ROUTING_LNK) $(LDFLAGS) $(EXEOUT)cvrptw$E

$(OBJ_DIR)/dobble_ls.$O:$(EX_DIR)examples/dobble_ls.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/dobble_ls.cc $(OBJ_OUT)dobble_ls.$O

$(BIN_DIR)/dobble_ls$E: $(CP_DEPS) $(OBJ_DIR)/dobble_ls.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/dobble_ls.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)dobble_ls$E

$(OBJ_DIR)/golomb.$O:$(EX_DIR)examples/golomb.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/golomb.cc $(OBJ_OUT)golomb.$O

$(BIN_DIR)/golomb$E: $(CP_DEPS) $(OBJ_DIR)/golomb.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/golomb.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)golomb$E

$(OBJ_DIR)/jobshop.$O:$(EX_DIR)examples/jobshop.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/jobshop.cc $(OBJ_OUT)jobshop.$O

$(BIN_DIR)/jobshop$E: $(CP_DEPS) $(OBJ_DIR)/jobshop.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/jobshop.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)jobshop$E

$(OBJ_DIR)/jobshop_ls.$O:$(EX_DIR)examples/jobshop_ls.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/jobshop_ls.cc $(OBJ_OUT)jobshop_ls.$O

$(BIN_DIR)/jobshop_ls$E: $(CP_DEPS) $(OBJ_DIR)/jobshop_ls.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/jobshop_ls.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)jobshop_ls$E

$(OBJ_DIR)/magic_square.$O:$(EX_DIR)examples/magic_square.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/magic_square.cc $(OBJ_OUT)magic_square.$O
...
$(OBJ_DIR)/model_util.$O:$(EX_DIR)examples/model_util.cc $(GEN_DIR)/constraint_solver/model.pb.h $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/model_util.cc $(OBJ_OUT)model_util.$O

$(BIN_DIR)/model_util$E: $(CP_DEPS) $(OBJ_DIR)/model_util.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/model_util.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)model_util$E

$(OBJ_DIR)/multidim_knapsack.$O:$(EX_DIR)examples/multidim_knapsack.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/multidim_knapsack.cc $(OBJ_OUT)multidim_knapsack.$O
...
$(OBJ_DIR)/network_routing.$O:$(EX_DIR)examples/network_routing.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/network_routing.cc $(OBJ_OUT)network_routing.$O
...
$(OBJ_DIR)/nqueens.$O: $(EX_DIR)examples/nqueens.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/nqueens.cc $(OBJ_OUT)nqueens.$O

$(BIN_DIR)/nqueens$E: $(CP_DEPS) $(OBJ_DIR)/nqueens.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/nqueens.$O $(CP_LNK) $(LDFLAGS) $(EXEOUT)nqueens$E

$(OBJ_DIR)/pdptw.$O: $(EX_DIR)examples/pdptw.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/pdptw.cc $(OBJ_OUT)pdptw.$O

$(BIN_DIR)/pdptw$E: $(ROUTING_DEPS) $(OBJ_DIR)/pdptw.$O
	$(CCC) $(CFLAGS) $(OBJ_DIR)/pdptw.$O $(ROUTING_LNK) $(LDFLAGS) $(EXEOUT)pdptw$E

$(OBJ_DIR)/sports_scheduling.$O:$(EX_DIR)examples/sports_scheduling.cc $(SRC_DIR)constraint_solver/constraint_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/sports_scheduling.cc $(OBJ_OUT)sports_scheduling.$O
...
$(OBJ_DIR)/tsp.$O: $(EX_DIR)examples/tsp.cc $(SRC_DIR)constraint_solver/routing.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/tsp.cc $(OBJ_OUT)tsp.$O
...
$(OBJ_DIR)/strawberry_fields_with_column_generation.$O: $(EX_DIR)examples/strawberry_fields_with_column_generation.cc $(SRC_DIR)linear_solver/linear_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/strawberry_fields_with_column_generation.cc $(OBJ_OUT)strawberry_fields_with_column_generation.$O
...
$(OBJ_DIR)/linear_programming.$O: $(EX_DIR)examples/linear_programming.cc $(SRC_DIR)linear_solver/linear_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/linear_programming.cc $(OBJ_OUT)linear_programming.$O
...
$(OBJ_DIR)/linear_solver_protocol_buffers.$O: $(EX_DIR)examples/linear_solver_protocol_buffers.cc $(SRC_DIR)linear_solver/linear_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/linear_solver_protocol_buffers.cc $(OBJ_OUT)linear_solver_protocol_buffers.$O
...
$(OBJ_DIR)/integer_programming.$O: $(EX_DIR)examples/integer_programming.cc $(SRC_DIR)linear_solver/linear_solver.h
	$(CCC) $(CFLAGS) -c $(EX_DIR)examples/integer_programming.cc $(OBJ_OUT)integer_programming.$O
