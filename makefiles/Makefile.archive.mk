...
ifeq ($(PLATFORM),WIN64)
	$(COPY) "lib$S$(LIB_PREFIX)flatzinc.$L*" "$(TEMP_FZ_DIR)$S$(FZ_INSTALL_DIR)$Slib"
else
	$(COPY) lib*$S$(LIB_PREFIX)flatzinc*.$L* "$(TEMP_FZ_DIR)$S$(FZ_INSTALL_DIR)$Slib"
endif
	$(COPY) bin$Sfz$E $(TEMP_FZ_DIR)$S$(FZ_INSTALL_DIR)$Sbin$S$(FZ_EXE)
	$(COPY) bin$Sparser_main$E $(TEMP_FZ_DIR)$S$(FZ_INSTALL_DIR)$Sbin$Sparser-or-tools$E
