...
//  This is a callback for a custom SearchLimit.
class LSInitialSolLimit : public ResultCallback<bool> {
 public:
  LSInitialSolLimit(Solver * solver,
  ...
  virtual bool Run() {
    ...
    if ((solver_->wall_time() - time_at_beginning_) > global_time_limit_) {
      limit_reached = true;
...
"Taillard's format with two basic local search operators.\n";
