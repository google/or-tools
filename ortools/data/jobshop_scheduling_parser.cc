...
void JsspParser::ProcessJsspLine(const std::string &line) {
  ...
  default: { LOG(FATAL) << "Should not be here with state " << parser_state_; }
  }
}
...
void JsspParser::ProcessSdstLine(const std::string &line) {
  ...
  case MACHINE_READ: {
    ...
    Machine *const machine = problem_.mutable_machines(current_machine_index_);
    ...
    if (++current_job_index_ == declared_job_count_) {
      parser_state_ =
          ++current_machine_index_ == declared_machine_count_ ? DONE : SSD_READ;
    }
    break;
  }
  default: {
    ...
    if (current_job_index_ == declared_job_count_) {
      ...
      for (const Job &job : problem_.jobs()) {
        if (job.lateness_cost_per_time_unit() %
                absl::GetFlag(FLAGS_jssp_scaling_up_factor) != 0) {
          all_integral = false;
          break;
        }
      }
      if (all_integral) {
        ...
        problem_.mutable_scaling_factor()
            ->set_value(1.0L / absl::GetFlag(FLAGS_jssp_scaling_up_factor));
      }
      parser_state_ = DONE;
    }
    break;
...
void JsspParser::ProcessPssLine(const std::string &line) {
    ...
    if (++current_machine_index_ == declared_machine_count_) {
      ...
      if (++current_job_index_ == declared_job_count_) {
        ...
        for (int m = 0; m < declared_machine_count_; ++m) {
          ...
          for (int i = 0; i < declared_job_count_ * declared_job_count_; ++i) {
  ...
  case JOBS_READ: {
    ...
    machine->mutable_transition_time_matrix()
        ->set_transition_time(job1 * declared_job_count_ + job2, transition);
