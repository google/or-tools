...
RcpspParser::RcpspParser()
    : seed_(-1), load_status_(NOT_STARTED), num_declared_tasks_(-1),
      current_task_(-1), unreads_(0) {
...
void RcpspParser::ProcessRcpspLine(const std::string &line) {
  ...
  case REQUEST_SECTION: {
    if (words[0] == "jobnr.") {
      ...
      Recipe *const recipe = rcpsp_.mutable_tasks(current_task_)->add_recipes();
  ...
  case PARSING_FINISHED: { break; }
  case ERROR_FOUND: { break; }
  }
}
