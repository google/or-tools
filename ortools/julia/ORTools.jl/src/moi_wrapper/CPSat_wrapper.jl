...
function MOI.add_variable(optimizer::CPSATOptimizer)
    throw(
        AddVariableNotAllowed(
            "CPSATOptimizer does not support adding nonconstrained variables. Use add_constrained_variable instead.",
        ),
    )
end
...
function MOI.add_constrained_variable(
    ...
    var_idx = length(optimizer.model.variables)
    lower_bound, upper_bound = bounds(set)
    push!(optimizer.model.variables, IntegerVariable([lower_bound, upper_bound]))

    return MOI.VariableIndex(var_idx)
end

function MOI.add_constrained_variable(optimizer::CPSATOptimizer, set::MOI.ZeroOne)
    var_idx = length(optimizer.model.variables)
    push!(optimizer.model.variables, IntegerVariable([Int(0), Int(1)]))
...
function MOI.set(
    ...
    optimizer.model.variables[v.value+1].name = name
...
function MOI.get(optimizer::CPSATOptimizer, ::MOI.VariableName, v::MOI.VariableIndex)
    if MOI.is_valid(optimizer, v)
        return optimizer.model.variables[v.value+1].name
    end
...
"""
  ...
  CpSolveResponse support.

"""

function MOI.get(optimizer::CPSATOptimizer, ::MOI.TerminationStatus)
    if isnothing(optimizer.solve_response)
        return MOI.OPTIMIZE_NOT_CALLED
    end

    if optimizer.solve_response.status == CpSolverStatus.OPTIMAL
        return MOI.OPTIMAL
    elseif optimizer.solve_response.status == CpSolverStatus.MODEL_INVALID
        return MOI.INVALID_MODEL
    elseif optimizer.solve_response.status == CpSolverStatus.INFEASIBLE
        return MOI.INFEASIBLE
    elseif optimizer.solve_response.status == CpSolverStatus.FEASIBLE
        # If it is a feasibility problem and we've found a feasible solution,
        # then our solution is optimal.
        if MOI.get(optimizer, MOI.ObjectiveSense) == MOI.FEASIBILITY_SENSE
            return MOI.OPTIMAL
        else
            return MOI.OTHER_LIMIT
        end 
    else
        # If the status is CpSolverStatus.UNKNOWN
        @info "The status of the model is still unknown. A search limit has been reached before the status could be determined."
        return MOI.OTHER_ERROR
    end
end

function MOI.get(optimizer::CPSATOptimizer, ::MOI.ResultCount)
    if isnothing(optimizer.solve_response)
        return 0
    end
    ...
    if !in(termination_status, [MOI.OPTIMAL, MOI.OTHER_LIMIT])
        # If one stopped exploring due to limits, we still count the number of
        # solutions found.
        number_of_solutions = !isempty(optimizer.solve_response.solution)
        if !isnothing(optimizer.parameters)
            if optimizer.parameters.fill_additional_solutions_in_response
                number_of_solutions += length(optimizer.solve_response.additional_solutions)
            end
        end
    ...
    if !isnothing(optimizer.parameters)
        # If either parameter was set for Optimization or Feasibility problem, get the length of available solutions.
        if optimizer.parameters.fill_additional_solutions_in_response
            return length(optimizer.solve_response.additional_solutions) + 1
        end
    end

    # Optimal or Feasible problem with no additional solutions.
    return 1
end

function MOI.get(optimizer::CPSATOptimizer, attr::MOI.ObjectiveValue)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    MOI.check_result_index_bounds(optimizer, attr)

    # TODO: b/506277218 -Get objective value based on result index

    termination_status = MOI.get(optimizer, MOI.TerminationStatus())
    # TODO: b/506281805 - Handle scenarios when a dual bound is present.
    if in(termination_status, [MOI.INFEASIBLE, MOI.INVALID_MODEL, MOI.OTHER_ERROR])
        throw(
            ArgumentError(
                "The objective value is not available for model with termination status: $termination_status.",
            ),
        )
    end

    return optimizer.solve_response.objective_value
end

function MOI.get(optimizer::CPSATOptimizer, ::MOI.ObjectiveBound)
    if isnothing(optimizer.solve_response)
        return nothing
    end
    ...
    if MOI.get(optimizer, MOI.ObjectiveSense()) == MOI.FEASIBILITY_SENSE
        throw(
            ArgumentError(
                "The objective bound is not available for feasibility problems.",
            ),
        )
    end
    ...
    if in(termination_status, [MOI.INFEASIBLE, MOI.INVALID_MODEL, MOI.OTHER_ERROR])
        throw(
            ArgumentError(
                "The objective bound is not available for model with termination status: $termination_status.",
            ),
        )
    end

    return optimizer.solve_response.best_objective_bound
end
...
function MOI.get(
    ...
    termination_status = MOI.get(optimizer, MOI.TerminationStatus())
    # No need to check for solutions if the Termination status wasn't optimal or at least feasible.
    if !in(termination_status, [MOI.OPTIMAL, MOI.OTHER_LIMIT])
        throw(
            ArgumentError(
                "The solution is only available for optimization problems or when feasibile solutions exist.",
            ),
        )
    end
    ...
    return optimizer.solve_response.solution.values[index.value-1]
end

struct SufficientAssumptionsForInfeasibility <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::SufficientAssumptionsForInfeasibility) =
    Union{Nothing,MOI.VectorOfVariables}
MOI.is_set_by_optimize(::SufficientAssumptionsForInfeasibility) = true

function MOI.get(optimizer::CPSATOptimizer, ::SufficientAssumptionsForInfeasibility)
    if isnothing(optimizer.solve_response) ||
        empty(optimizer.solve_response.sufficient_assumptions_for_infeasibility)
        return nothing
    end

    return MOI.VectorOfVariables(
        MOI.VariableIndex.(optimizer.solve_response.sufficient_assumptions_for_infeasibility),
    )
end

struct InnerObjectiveLowerBound <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::InnerObjectiveLowerBound) = Union{Nothing,Float64}
MOI.is_set_by_optimize(::InnerObjectiveLowerBound) = true

function MOI.get(optimizer::CPSATOptimizer, ::InnerObjectiveLowerBound)
    if isnothing(optimizer.solve_response)
        return nothing
    end

    return optimizer.solve_response.inner_objective_lower_bound
end

struct NumIntegers <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::NumIntegers) = Union{Nothing, Int64}
MOI.is_set_by_optimize(::NumIntegers) = true

function MOI.get(optimizer::CPSATOptimizer, ::NumIntegers)
    if isnothing(optimizer.solve_response)
        return nothing
    end

    # TODO: b/506281810 - update to num_integers
    return optimizer.solve_response.num_integer_variables
end

struct NumBooleans <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::NumBooleans) = Union{Nothing, Int64}
MOI.is_set_by_optimize(::NumBooleans) = true

function MOI.get(optimizer::CPSATOptimizer, ::NumBooleans)
    if isnothing(optimizer.solve_response)
        return nothing
    end

    return optimizer.solve_response.num_booleans
end

struct NumFixedBooleans <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::NumFixedBooleans) = Union{Nothing, Int64}
MOI.is_set_by_optimize(::NumFixedBooleans) = true

function MOI.get(optimizer::CPSATOptimizer, ::NumFixedBooleans)
    if isnothing(optimizer.solve_response)
        return nothing
    end

    return optimizer.solve_response.num_fixed_booleans
end

# TODO: b/506281811 - add comment to explain the difference between MOI.NodeCount
# and how it's being used in CPSAT which has no branch-and-bound tree.
function MOI.get(optimizer::CPSATOptimizer, ::MOI.NodeCount)
    if isnothing(optimizer.solve_response)
        return 0
    end

    return optimizer.solve_response.num_branches
end

struct NumBinaryPropagations <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::NumBinaryPropagations) = Int64
MOI.is_set_by_optimize(::NumBinaryPropagations) = true

function MOI.get(optimizer::CPSATOptimizer, ::NumBinaryPropagations)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.num_binary_propagations
end

struct NumIntegerPropagations <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::NumIntegerPropagations) = Int64
MOI.is_set_by_optimize(::NumIntegerPropagations) = true

function MOI.get(optimizer::CPSATOptimizer, ::NumIntegerPropagations)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.num_integer_propagations
end

struct NumRestarts <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::NumRestarts) = Int64
MOI.is_set_by_optimize(::NumRestarts) = true

function MOI.get(optimizer::CPSATOptimizer, ::NumRestarts)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.num_restarts
end

function MOI.get(optimizer::CPSATOptimizer, ::MOI.SimplexIterations)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.num_lp_iterations
end

function MOI.get(optimizer::CPSATOptimizer, ::MOI.SolveTimeSec)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.wall_time
end

struct UserTime <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::UserTime) = Float64
MOI.is_set_by_optimize(::UserTime) = true

function MOI.get(optimizer::CPSATOptimizer, ::UserTime)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.user_time
end

struct DeterministicTime <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::DeterministicTime) = Float64
MOI.is_set_by_optimize(::DeterministicTime) = true

function MOI.get(optimizer::CPSATOptimizer, ::DeterministicTime)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.deterministic_time
end

struct GapIntegral <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::GapIntegral) = Float64
MOI.is_set_by_optimize(::GapIntegral) = true

function MOI.get(optimizer::CPSATOptimizer, ::GapIntegral)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.gap_integral
end

function MOI.get(optimizer::CPSATOptimizer, ::MOI.RawStatusString)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    return optimizer.solve_response.solution_info
end

struct SolverLog <: MOI.AbstractOptimizerAttribute end
MOI.attribute_value_type(::SolverLog) = String
MOI.is_set_by_optimize(::SolverLog) = true

function MOI.get(optimizer::CPSATOptimizer, ::SolverLog)
    if isnothing(optimizer.solve_response)
        throw(ArgumentError("The solve response is not set. Call optimize! first."))
    end

    if isnothing(optimizer.parameters) || !optimizer.parameters.log_to_response
        throw(
            ArgumentError(
                "To get the solver log, the log_to_response parameter must be set to true before solving.",
            ),
        )
    end

    return optimizer.solve_response.solver_log
end

function MOI.optimize!(optimizer::CPSATOptimizer)
    # Serialize the model
    io = IOBuffer()
    e = PB.ProtoEncoder(io)
    PB.encode(e, to_proto_struct(optimizer.model))
    model_proto = take!(io)
    model_size = encoded_model_size(optimizer.model)[1]

    # Serialize the parameters
    io = IOBuffer()
    e = PB.ProtoEncoder(io)
    PB.encode(e, to_proto_struct(optimizer.parameters))
    parameters_proto = take!(io)
    parameters_size = encoded_parameters_size(optimizer.parameters)[1]

    solve_response_proto = Ref{Ptr{Cvoid}}()
    response_size = Ref{Csize_t}(0)

    cp_solve_response = SolveCpModelWithParameters(
        model_proto,
        model_size,
        parameters_proto,
        parameters_size,
        solve_response_proto,
        response_size,
    )

    solve_response_proto =
        unsafe_wrap(Vector{UInt8}, Ptr{UInt8}(solve_response_proto[]), response_size[])
    io = IOBuffer(solve_response_proto)
    d = PB.ProtoDecoder(io)
    optimizer.solve_response = PB.decode(d, CpSolverResponse)
