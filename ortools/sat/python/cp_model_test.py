...
class CpModelTest(absltest.TestCase):
  ...
  def test_names(self) -> None:
    ...
    x = model.new_int_var(-10, 10, "x")
    y = model.new_int_var(-10, 10, "y")
    ct = model.add_linear_constraint(x + 2 * y, 0, 10).with_name(
        "test_constraint"
    )
  ...
  def test_element(self) -> None:
    ...
    self.assertEqual(
        4, model.proto.constraints[0].element.linear_target.vars[0]
    )
    with self.assertRaises(ValueError):
  ...
  def test_affine_element(self) -> None:
    ...
    self.assertEqual(0, model.proto.constraints[0].element.linear_index.vars[0])
    self.assertEqual(
        1, model.proto.constraints[0].element.linear_index.coeffs[0]
    )
    ...
    self.assertEqual(
        1, model.proto.constraints[0].element.linear_target.coeffs[0]
    )
    self.assertEqual(
        -1, model.proto.constraints[0].element.linear_target.offset
    )
    self.assertEqual(
        4, model.proto.constraints[0].element.linear_target.vars[0]
    )
  ...
  def test_automaton(self) -> None:
    ...
    x = [model.new_int_var(0, 4, f"x{i}") for i in range(5)]
    model.add_automaton(
        x, 0, [2, 3], [(0, 0, 0), (0, 1, 1), (1, 2, 2), (2, 3, 3)]
    )
    self.assertLen(model.proto.variables, 5)
    self.assertLen(model.proto.constraints, 1)
  ...
  def test_str(self) -> None:
    ...
    b = model.new_bool_var("b")
    self.assertEqual(
        str(cp_model.LinearExpr.term(b.negated(), 3)), "(3 * not(b))"
    )
  ...
  def test_repr(self) -> None:
    ...
    self.assertEqual(
        repr(j), "j(start = x1, size = 2, end = y1, is_present = b)"
    )
  ...
  def testDisplayBounds(self) -> None:
    ...
    self.assertEqual(
        "10..15, 20..30", cp_model.display_bounds([10, 15, 20, 30])
    )
  ...
  def test_search_strategy(self) -> None:
    ...
    self.assertEqual(
        cp_model.SELECT_MAX_VALUE, strategy.domain_reduction_strategy
    )
  ...
  def test_model_error(self) -> None:
    ...
    self.assertEqual(
        solver.solution_info(), 'var #0 has no domain(): name: "x0"'
    )
  ...
  def test_fixed_size_interval_var_series(self) -> None:
    ...
    opt_fixed_size_intervals = (
        model.new_optional_fixed_size_interval_var_series(
            name="fixed_size_intervals",
            index=df.index,
            starts=starts,
            sizes=df.size,
...
TRFM"""
    ...
    for i in range(len(tiles)):
      for x in range(6):
        for y in range(6):
          for r in range(4):
            choices[(i, x, y, r)] = model.new_bool_var(f"tile_{i}_{x}_{y}_{r}")
