...
# Type aliases
IntegralT = Union[
    int, np.int8, np.uint8, np.int32, np.uint32, np.int64, np.uint64
]
IntegralTypes = (
    int,
    np.int8,
...
def object_is_a_true_literal(literal: LiteralT) -> bool:
  ...
  if isinstance(literal, IntVar):
    proto = literal.proto
    return (
        len(proto.domain) == 2 and proto.domain[0] == 1 and proto.domain[1] == 1
    )
  if isinstance(literal, cmh.NotBooleanVariable):
    proto = literal.negated().proto
    return (
        len(proto.domain) == 2 and proto.domain[0] == 0 and proto.domain[1] == 0
    )
...
class CpModel:
  ...
  * ```add``` create new constraints and add them to the model.
  ...
  def add(
      self, ct: Union[BoundedLinearExpression, bool, np.bool_]
  ) -> Constraint:
  ...
  def add_element(
    ...
    model_ct.element.linear_target.CopyFrom(
        self.parse_linear_expression(target)
    )
    return ct
  ...
  def add_inverse(
    ...
    model_ct.inverse.f_direct.extend(
        [self.get_or_make_index(x) for x in variables]
    )
  ...
  def add_max_equality(
    ...
    model_ct = self.__model.constraints[ct.index]
    model_ct.lin_max.exprs.extend(
        [self.parse_linear_expression(x) for x in exprs]
    )
  ...
  def add_abs_equality(
      self, target: LinearExprT, expr: LinearExprT
  ) -> Constraint:
  ...
  def new_optional_interval_var_series(
    ...
    sizes = _convert_to_linear_expr_series_and_validate_index(sizes, index)
    ends = _convert_to_linear_expr_series_and_validate_index(ends, index)
    are_present = _convert_to_literal_series_and_validate_index(
        are_present, index
    )
  ...
  def add_cumulative(
    ...
    model_ct.cumulative.capacity.CopyFrom(
        self.parse_linear_expression(capacity)
    )
    return cumulative
  ...
  def get_interval_var_from_proto_index(self, index: int) -> IntervalVar:
    ...
    return IntervalVar(
        self.__model, self.__var_list, index, None, None, None, None
    )
  ...
  def get_or_make_boolean_index(self, arg: LiteralT) -> int:
    ...
    raise TypeError(
        "not supported:"
        f" model.get_or_make_boolean_index({type(arg).__name__!r})"
    )
  ...
  NewOptionalFixedSizeIntervalVarSeries = (
...
class CpSolver:
  ...
  def float_values(self, expressions: _IndexOrSeries) -> pd.Series:
    ...
    return pd.Series(
        data=[
            self.__response_wrapper.float_value(expr) for expr in expressions
        ],
  ...
  def boolean_values(self, variables: _IndexOrSeries) -> pd.Series:
    ...
    return pd.Series(
        data=[
            self.__response_wrapper.boolean_value(literal)
