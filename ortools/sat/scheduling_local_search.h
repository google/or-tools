...
// search. The heuristic this class uses for single-machine moves is the Nowicki
// & Smutnicki algorithm described in "Eugeniusz Nowicki, Czeslaw Smutnicki,
// (1996) A Fast Taboo Search Algorithm for the Job Shop Problem. Management
// Science 42(6):797-813".
class SchedulingLocalSearch {
 public:
  ...
  LocalSearchResult Solve(absl::Span<const int64_t> initial_hint,
 ...
 protected:  // Exposed for testing.
  ...
  struct InsertMove {
    ...
    bool place_after;  // true: insert after target_task; false: insert before
                       // target_task
    int new_machine;   // The machine this task is being assigned to
    int64_t new_duration;  // This could be recomputed, but we cache it here for
                           // saving some CPU.
  };
  ...
  struct SolverState {
    ...
    std::vector<int64_t> machine_workloads;
  ...
  // Generates unified sequence and assignment moves.
  std::vector<InsertMove> GenerateMoves(
  ...
  void GenerateN8Moves(absl::Span<const int> critical_path,
  ...
  void GenerateNAMoves(absl::Span<const int> critical_path,
                       absl::Span<const IntegerValue> start_mins,
                       absl::Span<const IntegerValue> tails,
                       absl::Span<const int> current_machines,
                       absl::Span<const int64_t> current_durations,
  ...
  void ApplySequenceMove(const InsertMove& move, const SolverState& state,
                         int current_iteration, int tenure,
                         absl::Span<int> tabu_matrix,
                         absl::Span<int> machine_seq) const;

  void ApplyAssignmentMove(
  ...
  IntegerValue EstimateSequenceMove(
  ...
  IntegerValue EstimateAssignmentMove(
      const InsertMove& move, absl::Span<const IntegerValue> start_mins,
  ...
  IntegerValue ComputeMutatedSequenceMakespan(
