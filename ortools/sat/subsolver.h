...
// Simple framework for choosing and distributing solver "sub-tasks" on a set
// of threads.
...
//
// Note that currently only the main thread interacts with subsolvers. Only the
...
class SubSolver {
 public:
  ...
  // Synchronizes with the external world from this SubSolver's point of view.
  // Also incorporates the results of the latest completed tasks if any.
  //
  ...
  // that the *Loop(subsolvers) functions below take a reference in order to be
  ...
  //
  // This is needed since some subsolvers can be done before the overall Solve()
  // is finished. This is the case for first solution subsolvers, for instance.
  //
  ...
  // corresponds to the number of total calls to GenerateTask().
  //
  ...
  // Returns the total deterministic time spent by the completed tasks before
  ...
  // This one needs to be called by the subclasses. Usually from Synchronize(),
  // or from the task itself if we execute a single task at the same time.
  ...
  double GetSelectionScore(bool deterministic) const {
    ...
    // If we have little data, we strongly limit the number of tasks in flight.
    // This is needed if some LNS are stuck for a long time to not only
    ...
    // We assume a "minimum time per task" which will be our base estimation for
...
//
// Note that it is okay to incorporate "special" subsolvers that never produce
...
// deterministic solver provided that all SubSolvers respect the Synchronize()
// contract.
//
// Executes the following loop:
// 1/ Synchronize all in given order.
// 2/ generate and schedule up to batch_size tasks using a heuristic to select
//    which one to run.
...
// as the functions above with num_threads=1 and batch_size=1. Note that a
