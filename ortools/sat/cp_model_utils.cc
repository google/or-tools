...
#define APPLY_TO_REPEATED_FIELD(ct_name, field_name)                           \
  {                                                                            \
    for (int &r : *ct->mutable_##ct_name()->mutable_##field_name())            \
      f(&r);                                                                   \
  }

void ApplyToAllLiteralIndices(const std::function<void(int *)> &f,
                              ConstraintProto *ct) {
  for (int &r : *ct->mutable_enforcement_literal())
    f(&r);
  switch (ct->constraint_case()) {
  case ConstraintProto::ConstraintCase::kBoolOr:
...
std::string
ConstraintCaseName(ConstraintProto::ConstraintCase constraint_case) {
...
int64 ComputeInnerObjective(const CpObjectiveProto &objective,
  ...
  auto &repeated_field_values =
      response.solution().empty() ? response.solution_lower_bounds()
                                  : response.solution();
