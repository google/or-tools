...
TEST(IntervalsRepositoryTest, Precedences) {
  ...
  EXPECT_NE(kNoLiteralIndex, repo->GetPrecedenceLiteral(a, b));
  EXPECT_EQ(Literal(repo->GetPrecedenceLiteral(a, b)),
            Literal(repo->GetPrecedenceLiteral(b, a)).Negated());
}
...
TEST(SchedulingConstraintHelperTest, PushConstantBoundWithOptionalIntervals) {
  ...
  SchedulingConstraintHelper helper({inter1, inter2}, &model);
...
TEST(SchedulingDemandHelperTest, EnergyInWindow) {
  ...
  const IntervalVariable inter =
      model.GetOrCreate<IntervalsRepository>()->CreateInterval(
          start, end, size, kNoLiteralIndex, false);

  const AffineExpression demand(model.Add(NewIntegerVariable(2, 10)));

  SchedulingConstraintHelper helper({inter}, &model);
