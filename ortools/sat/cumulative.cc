...
void AddCumulativeTimeDecomposition(
    ...
    Model* model, SchedulingConstraintHelper* /*helper*/) {
  CHECK(enforcement_literals.empty());
  if (vars.empty()) return;

  IntegerTrail* integer_trail = model->GetOrCreate<IntegerTrail>();
  CHECK(integer_trail->IsFixed(capacity));
  const Coefficient fixed_capacity(integer_trail->UpperBound(capacity).value());
  ...
  for (int t = 0; t < num_tasks; ++t) {
    min_start = std::min(min_start, integer_trail->LowerBound(start_exprs[t]));
  ...
  for (IntegerValue time = min_start; time < max_end; ++time) {
    ...
    for (int t = 0; t < num_tasks; ++t) {
      ...
      const IntegerValue start_min = integer_trail->LowerBound(start_exprs[t]);
    ...
    sat_solver->AddLinearConstraint(false, Coefficient(0), true, fixed_capacity,
                                    &literals_with_coeff);
