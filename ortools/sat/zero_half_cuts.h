...
#define ORTOOLS_SAT_ZERO_HALF_CUTS_H_
...
// Heuristic to find a good sum of rows from the LP (with coeff -1, +1) that
// can lead to a violated zero-half cut (i.e. after integer rounding with a
// divisor 2).
//
// For this, all that matters is the parity of the coefficients and the rhs in
// the linear combination of the original problem constraints. So this class
// maintains a copy of the LP matrix modulo 2 on which simplifications and
// heuristics are performed to find good cut candidate(s).
//
...
class ZeroHalfCutHelper {
 public:
  ...
  // code performance can probably be improved upon.
  ...
  // Visible for testing.
  //
  // Boolean matrix. Each column corresponds to one variable (col indices).
  // Each row corresponds to a sum of the initial problem constraints. We store
  // the coefficient modulo 2, so only the positions of the ones.
  struct CombinationOfRows {
    ...
    // The indices of the odd coefficients of this combination.
    std::vector<int> cols;

    // The parity of the rhs (1 for odd).
    int rhs_parity;

    // How tight this constraint is under the current LP solution.
    double slack;
  };
  ...
  // Visible for testing.
  //
  // Adds the given row to all other rows having an odd coefficient on the given
  // column. This then eliminates the entry (col, row) that is now a singleton
  // by increasing the slack of the given row.
  ...
  // speed, but it is DCHECKed on each EliminateVarUsingRow() call.
  void SymmetricDifference(absl::Span<const int> a, std::vector<int>* b);

 private:
  // As we combine rows, when the activity of a combination gets too far away
  ...
  // We don't consider long constraints or constraints with high magnitude,
  // since the highest violation we can hope for is 1, and if the magnitude is
  // large then the cut efficacy will not be great.
...
#endif  // ORTOOLS_SAT_ZERO_HALF_CUTS_H_
