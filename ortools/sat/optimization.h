...
#ifndef ORTOOLS_SAT_OPTIMIZATION_H_
#define ORTOOLS_SAT_OPTIMIZATION_H_
...
// be inferred by propagation by any subset of the other literals, it will be
// removed.
//
// Note that the literals of the minimized core will stay in the same order.
//
...
// decision problems. Each problem is solved using SolveIntegerProblem().
// Returns the status of the last solved decision problem.
//
...
// Uses a low conflict limit and performs a binary search to try to restrict the
...
// Same as MinimizeIntegerVariableWithLinearScanAndLazyEncoding() but uses
// a core-based approach instead. Note that the given objective_var is just used
// for reporting the lower-bound/upper-bound and does not need to be linked with
// its linear representation.
//
// Unlike MinimizeIntegerVariableWithLinearScanAndLazyEncoding() this function
// just returns the last solver status. In particular if it is INFEASIBLE but
...
class CoreBasedOptimizer {
 public:
  ...
  // TODO(user): Change the algorithm slightly to allow resuming from the last
  ...
  // A different way to encode the objective as cores are found.
  //
  // If the vector of literals is passed it will use that, otherwise it will
 ...
 private:
  ...
  // Use the gap and implied bounds to propagate the bounds of the objective
  // variables and of its terms.
  bool PropagateObjectiveBounds();

  // Heuristic that aims to find the "real" lower bound of the objective on each
  // core by using a linear scan optimization approach.
  bool CoverOptimization();

  // Computes the next stratification threshold.
  // Sets it to zero if all the assumptions were already considered.
  ...
  //
  // This detects such "at most ones" and rewrites the objective accordingly.
  // For each at most one, the rewrite creates a new Boolean variable and
  // updates the cost so that the trivial objective lower bound reflects the
  // increase.
  //
  ...
  // will see the same lower bound than what is computed by this.
...
#endif  // ORTOOLS_SAT_OPTIMIZATION_H_
