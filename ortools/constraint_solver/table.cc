...
AffineTransformation::AffineTransformation() : a(1), b(0) {}
AffineTransformation::AffineTransformation(int64_t aa, int64_t bb)
    : a(aa), b(bb) {
  CHECK_NE(a, 0);
}

bool AffineTransformation::Reverse(int64_t value, int64_t* reverse) const {
...
int64_t AffineTransformation::Forward(int64_t value) const {
  return value * a + b;
}

int64_t AffineTransformation::UnsafeReverse(int64_t value) const {
  return (value - b) / a;
}

void AffineTransformation::Clear() {
  a = 1;
  b = 0;
}

std::string AffineTransformation::DebugString() const {
  return absl::StrFormat("(%d * x + %d)", a, b);
}

VarLinearizer::VarLinearizer()
    : target_var_(nullptr), transformation_(nullptr) {}
VarLinearizer::~VarLinearizer() {}

void VarLinearizer::VisitIntegerVariable(const IntVar* variable,
...
void VarLinearizer::Visit(const IntVar* var, IntVar** target_var,
...
std::string VarLinearizer::DebugString() const { return "VarLinearizer"; }

void VarLinearizer::AddConstant(int64_t constant) {
  transformation_->b += constant * multipliers_.back();
}

void VarLinearizer::PushMultiplier(int64_t multiplier) {
...
void VarLinearizer::PopMultiplier() { multipliers_.pop_back(); }

// ----- Positive Table Constraint -----

BasePositiveTableConstraint::BasePositiveTableConstraint(
    Solver* s, const std::vector<IntVar*>& vars, const IntTupleSet& tuples)
    : Constraint(s),
...
std::string BasePositiveTableConstraint::DebugString() const {
  return absl::StrFormat("AllowedAssignments(arity = %d, tuple_count = %d)",
                         arity_, tuple_count_);
}

void BasePositiveTableConstraint::Accept(ModelVisitor* visitor) const {
...
bool BasePositiveTableConstraint::TupleValue(int tuple_index, int var_index,
                                             int64_t* value) const {
  return transformations_[var_index].Reverse(
      tuples_.Value(tuple_index, var_index), value);
}

int64_t BasePositiveTableConstraint::UnsafeTupleValue(int tuple_index,
                                                      int var_index) const {
  return transformations_[var_index].UnsafeReverse(
      tuples_.Value(tuple_index, var_index));
}

bool BasePositiveTableConstraint::IsTupleSupported(int tuple_index) {
...
PositiveTableConstraint::PositiveTableConstraint(
    Solver* s, const std::vector<IntVar*>& vars, const IntTupleSet& tuples)
...
void PositiveTableConstraint::Post() {
...
void PositiveTableConstraint::InitialPropagate() {
...
void PositiveTableConstraint::Propagate() {
  for (int var_index = 0; var_index < arity_; ++var_index) {
    IntVar* const var = vars_[var_index];
    to_remove_.clear();
    for (const int64_t value : InitAndGetValues(iterators_[var_index])) {
...
void PositiveTableConstraint::Update(int var_index) {
  IntVar* const var = vars_[var_index];
  const int64_t old_max = var->OldMax();
  const int64_t vmin = var->Min();
  const int64_t vmax = var->Max();
  const ValueBitset& var_masks = masks_[var_index];
...
void PositiveTableConstraint::BlankActives(const std::vector<uint64_t>& mask) {
  if (!mask.empty()) {
...
bool PositiveTableConstraint::Supported(int var_index, int64_t value) {
...
std::string PositiveTableConstraint::DebugString() const {
  return absl::StrFormat("PositiveTableConstraint([%s], %d tuples)",
                         JoinDebugStringPtr(vars_), tuple_count_);
}

void PositiveTableConstraint::InitializeMask(int tuple_index) {
...
CompactPositiveTableConstraint::CompactPositiveTableConstraint(
    Solver* s, const std::vector<IntVar*>& vars, const IntTupleSet& tuples)
    : BasePositiveTableConstraint(s, vars, tuples),
      word_length_(BitLength64(tuples.NumTuples())),
      active_tuples_(tuples.NumTuples()),
...
void CompactPositiveTableConstraint::Post() {
  demon_ = solver()->RegisterDemon(MakeDelayedConstraintDemon0(
      solver(), this, &CompactPositiveTableConstraint::Propagate, "Propagate"));
  for (int i = 0; i < arity_; ++i) {
...
void CompactPositiveTableConstraint::InitialPropagate() {
  BuildMasks();
...
void CompactPositiveTableConstraint::Propagate() {
...
void CompactPositiveTableConstraint::Update(int var_index) {
...
std::string CompactPositiveTableConstraint::DebugString() const {
  return absl::StrFormat("CompactPositiveTableConstraint([%s], %d tuples)",
                         JoinDebugStringPtr(vars_), tuple_count_);
}

void CompactPositiveTableConstraint::BuildMasks() {
  // Build masks.
  for (int i = 0; i < arity_; ++i) {
...
void CompactPositiveTableConstraint::FillMasksAndActiveTuples() {
  std::vector<uint64_t> actives(word_length_, 0);
  for (int tuple_index = 0; tuple_index < tuple_count_; ++tuple_index) {
    if (IsTupleSupported(tuple_index)) {
      SetBit64(actives.data(), tuple_index);
...
void CompactPositiveTableConstraint::RemoveUnsupportedValues() {
  // remove unreached values.
  for (int var_index = 0; var_index < arity_; ++var_index) {
    IntVar* const var = vars_[var_index];
    to_remove_.clear();
...
void CompactPositiveTableConstraint::ComputeMasksBoundaries() {
  for (int var_index = 0; var_index < arity_; ++var_index) {
...
void CompactPositiveTableConstraint::BuildSupports() {
  for (int var_index = 0; var_index < arity_; ++var_index) {
    supports_[var_index].resize(masks_[var_index].size());
  }
}

bool CompactPositiveTableConstraint::AndMaskWithActive(
...
bool CompactPositiveTableConstraint::SubtractMaskFromActive(
    const std::vector<uint64_t>& mask) {
...
bool CompactPositiveTableConstraint::Supported(int var_index,
                                               int64_t value_index) {
  DCHECK_GE(var_index, 0);
  DCHECK_LT(var_index, arity_);
  DCHECK_GE(value_index, 0);
...
void CompactPositiveTableConstraint::OrTempMask(int var_index,
                                                int64_t value_index) {
  const std::vector<uint64_t>& mask = masks_[var_index][value_index];
  if (!mask.empty()) {
...
void CompactPositiveTableConstraint::SetTempMask(int var_index,
                                                 int64_t value_index) {
...
void CompactPositiveTableConstraint::ClearTempMask() {
  // See comment above.
  if (active_tuples_.ActiveWordSize() < word_length_ / 4) {
    for (int i : active_tuples_.active_words()) {
      temp_mask_[i] = 0;
...
SmallCompactPositiveTableConstraint::SmallCompactPositiveTableConstraint(
    Solver* s, const std::vector<IntVar*>& vars, const IntTupleSet& tuples)
    : BasePositiveTableConstraint(s, vars, tuples),
      active_tuples_(0),
      stamp_(0),
...
void SmallCompactPositiveTableConstraint::Post() {
  demon_ = solver()->RegisterDemon(MakeDelayedConstraintDemon0(
...
void SmallCompactPositiveTableConstraint::InitMasks() {
  // Build masks.
  for (int i = 0; i < arity_; ++i) {
    original_min_[i] = vars_[i]->Min();
    const int64_t span = vars_[i]->Max() - original_min_[i] + 1;
...
bool SmallCompactPositiveTableConstraint::IsTupleSupported(int tuple_index) {
  for (int var_index = 0; var_index < arity_; ++var_index) {
    int64_t value = 0;
    if (!TupleValue(tuple_index, var_index, &value) ||
        !vars_[var_index]->Contains(value)) {
...
void SmallCompactPositiveTableConstraint::ComputeActiveTuples() {
  active_tuples_ = 0;
...
void SmallCompactPositiveTableConstraint::RemoveUnsupportedValues() {
  // remove unreached values.
  for (int var_index = 0; var_index < arity_; ++var_index) {
    IntVar* const var = vars_[var_index];
    const int64_t original_min = original_min_[var_index];
...
void SmallCompactPositiveTableConstraint::InitialPropagate() {
  InitMasks();
  ComputeActiveTuples();
  RemoveUnsupportedValues();
}

void SmallCompactPositiveTableConstraint::Propagate() {
...
void SmallCompactPositiveTableConstraint::Update(int var_index) {
...
std::string SmallCompactPositiveTableConstraint::DebugString() const {
  return absl::StrFormat("SmallCompactPositiveTableConstraint([%s], %d tuples)",
                         JoinDebugStringPtr(vars_), tuple_count_);
}

void SmallCompactPositiveTableConstraint::ApplyMask(int var_index,
                                                    uint64_t mask) {
...
TransitionConstraint::TransitionConstraint(
    ...
    const IntTupleSet& transition_table, int64_t initial_state,
...
TransitionConstraint::TransitionConstraint(Solver* s,
  ...
  for (int var_index = 0; var_index < nb_vars; ++var_index) {
    ...
    if (num_tuples <= kBitsInUint64) {
      ...
      s->AddConstraint(s->RevAlloc(
          new CompactPositiveTableConstraint(s, tmp_vars, transition_table_)));
    }
  }
}

void TransitionConstraint::InitialPropagate() {}

void TransitionConstraint::Accept(ModelVisitor* visitor) const {
...
std::string TransitionConstraint::DebugString() const {
  return absl::StrFormat(
      "TransitionConstraint([%s], %d transitions, initial = %d, final = "
      "[%s])",
      JoinDebugStringPtr(vars_), transition_table_.NumTuples(), initial_state_,
      absl::StrJoin(final_states_, ", "));
}
