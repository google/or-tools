...
DomainWatcher::DomainWatcher(const std::vector<IntVar*>& vars, int cache_size)
    : vars_(vars) {
  cached_log_.Init(cache_size);
}

double DomainWatcher::LogSearchSpaceSize() {
  double result = 0.0;
...
double DomainWatcher::Log2(int64_t size) const {
  return cached_log_.Log2(size);
}

// ---------- FindVar decision visitor ---------

FindVar::FindVar() : var_(nullptr), value_(0), operation_(NONE) {}

FindVar::~FindVar() {}

void FindVar::VisitSetVariableValue(IntVar* var, int64_t value) {
  var_ = var;
  value_ = value;
  operation_ = ASSIGN;
}

void FindVar::VisitSplitVariableDomain(IntVar* var, int64_t value,
...
void FindVar::VisitScheduleOrPostpone(IntervalVar* var, int64_t est) {
  operation_ = NONE;
}

void FindVar::VisitTryRankFirst(SequenceVar* sequence, int index) {
  operation_ = NONE;
}

void FindVar::VisitTryRankLast(SequenceVar* sequence, int index) {
  operation_ = NONE;
}

void FindVar::VisitUnknownDecision() { operation_ = NONE; }

IntVar* FindVar::var() const {
  CHECK_NE(operation_, NONE);
  return var_;
}

int64_t FindVar::value() const {
  CHECK_NE(operation_, NONE);
  return value_;
}

FindVar::Operation FindVar::operation() const { return operation_; }

std::string FindVar::DebugString() const { return "FindVar decision visitor"; }

// ----- Auxiliary decision builders to init impacts -----

InitVarImpacts::InitVarImpacts()
    : var_(nullptr),
...
void InitVarImpacts::UpdateImpacts() {
  // the Min is always the value we just set.
  update_impact_callback_(var_index_, var_->Min());
}

void InitVarImpacts::Init(IntVar* var, IntVarIterator* iterator,
                          int var_index) {
  var_ = var;
  iterator_ = iterator;
...
Decision* InitVarImpacts::Next(Solver* solver) {
  CHECK(var_ != nullptr);
...
void InitVarImpacts::set_update_impact_callback(
    std::function<void(int, int64_t)> callback) {
  update_impact_callback_ = std::move(callback);
}

InitVarImpacts::AssignCallFail::AssignCallFail(
    const std::function<void()>& update_impact_closure)
    : var_(nullptr), value_(0), update_impact_closure_(update_impact_closure) {
  CHECK(update_impact_closure_ != nullptr);
}

InitVarImpacts::AssignCallFail::~AssignCallFail() {}

void InitVarImpacts::AssignCallFail::Apply(Solver* solver) {
  CHECK(var_ != nullptr);
...
void InitVarImpacts::AssignCallFail::Refute(Solver* solver) {}

// ... InitVarImpactsWithSplits ...

InitVarImpactsWithSplits::InitVarImpactsWithSplits(int split_size)
    : var_(nullptr),
      update_impact_callback_(nullptr),
      new_start_(false),
      var_index_(0),
...
InitVarImpactsWithSplits::~InitVarImpactsWithSplits() {}

void InitVarImpactsWithSplits::UpdateImpacts() {
  for (const int64_t value : InitAndGetValues(iterator_)) {
    update_impact_callback_(var_index_, value);
  }
}

void InitVarImpactsWithSplits::Init(IntVar* var, IntVarIterator* iterator,
                                    int var_index) {
  var_ = var;
  iterator_ = iterator;
  var_index_ = var_index;
...
int64_t InitVarImpactsWithSplits::IntervalStart(int index) const {
  const int64_t length = max_value_ - min_value_ + 1;
  return (min_value_ + length * index / split_size_);
}

Decision* InitVarImpactsWithSplits::Next(Solver* solver) {
  if (new_start_) {
    min_value_ = var_->Min();
    max_value_ = var_->Max();
    new_start_ = false;
...
void InitVarImpactsWithSplits::set_update_impact_callback(
    std::function<void(int, int64_t)> callback) {
  update_impact_callback_ = std::move(callback);
}

InitVarImpactsWithSplits::AssignIntervalCallFail::AssignIntervalCallFail(
    const std::function<void()>& update_impact_closure)
    : var_(nullptr),
      value_min_(0),
      value_max_(0),
...
void InitVarImpactsWithSplits::AssignIntervalCallFail::Apply(Solver* solver) {
  CHECK(var_ != nullptr);
...
void InitVarImpactsWithSplits::AssignIntervalCallFail::Refute(Solver* solver) {}
...
ImpactRecorder::ImpactRecorder(
    Solver* solver, DomainWatcher* domain_watcher,
...
void ImpactRecorder::ApplyDecision(Decision* d) {
  if (!init_done_) {
    return;
  }
  d->Accept(&find_var_);
...
void ImpactRecorder::AfterDecision(Decision* d, bool apply) {
...
void ImpactRecorder::ResetAllImpacts() {
  for (int i = 0; i < size_; ++i) {
...
void ImpactRecorder::UpdateImpact(int var_index, int64_t value, double impact) {
  ...
  const double new_impact =
      (current_impact * (absl::GetFlag(FLAGS_cp_impact_divider) - 1) + impact) /
      absl::GetFlag(FLAGS_cp_impact_divider);
  impacts_[var_index][value_index] = new_impact;
}

void ImpactRecorder::InitImpact(int var_index, int64_t value) {
  const double log_space = domain_watcher_->LogSearchSpaceSize();
  const double impact = kPerfectImpact - log_space / current_log_space_;
  const int64_t value_index = value - original_min_[var_index];
...
void ImpactRecorder::FirstRun(int64_t splits) {
  Solver* s = solver();
  current_log_space_ = domain_watcher_->LogSearchSpaceSize();
...
void ImpactRecorder::ScanVarImpacts(
    int var_index, int64_t* best_impact_value, double* var_impacts,
...
std::string ImpactRecorder::DebugString() const { return "ImpactRecorder"; }

ImpactRecorder::FirstRunVariableContainers::FirstRunVariableContainers(
    ImpactRecorder* impact_recorder, int64_t splits)
    : update_impact_callback_([impact_recorder](int var_index, int64_t value) {
...
ImpactRecorder::FirstRunVariableContainers::update_impact_callback() const {
  return update_impact_callback_;
}

void ImpactRecorder::FirstRunVariableContainers::PushBackRemovedValue(
    int64_t value) {
  removed_values_.push_back(value);
}

bool ImpactRecorder::FirstRunVariableContainers::HasRemovedValues() const {
  return !removed_values_.empty();
}

void ImpactRecorder::FirstRunVariableContainers::ClearRemovedValues() {
  removed_values_.clear();
}

size_t ImpactRecorder::FirstRunVariableContainers::NumRemovedValues() const {
  return removed_values_.size();
}

const std::vector<int64_t>&
ImpactRecorder::FirstRunVariableContainers::removed_values() const {
  return removed_values_;
}

InitVarImpacts* ImpactRecorder::FirstRunVariableContainers::without_split() {
  return &without_splits_;
}

InitVarImpactsWithSplits*
ImpactRecorder::FirstRunVariableContainers::with_splits() {
  return &with_splits_;
}

std::string ImpactRecorder::FirstRunVariableContainers::DebugString() const {
  return "FirstRunVariableContainers";
}

// ... RunHeuristicsAsDives ...

RunHeuristicsAsDives::RunHeuristicsAsDives(
    Solver* solver, const std::vector<IntVar*>& vars,
    DefaultPhaseParameters::DisplayLevel level, bool run_all_heuristics,
    int random_seed, int heuristic_period, int heuristic_num_failures_limit)
...
RunHeuristicsAsDives::~RunHeuristicsAsDives() {
  gtl::STLDeleteElements(&heuristics_);
}

void RunHeuristicsAsDives::Apply(Solver* solver) {
  if (!RunAllHeuristics(solver)) {
    solver->Fail();
  }
}

void RunHeuristicsAsDives::Refute(Solver* solver) {}

bool RunHeuristicsAsDives::ShouldRun() {
...
bool RunHeuristicsAsDives::RunOneHeuristic(Solver* solver, int index) {
  HeuristicWrapper* const wrapper = heuristics_[index];
  heuristic_runs_++;

  const bool result = solver->SolveAndCommit(wrapper->phase, heuristic_limit_);
...
bool RunHeuristicsAsDives::RunAllHeuristics(Solver* solver) {
...
int RunHeuristicsAsDives::Rand32(int size) {
  DCHECK_GT(size, 0);
  return absl::Uniform<int>(random_, 0, size);
}

void RunHeuristicsAsDives::Init(Solver* solver,
                                const std::vector<IntVar*>& vars,
...
int RunHeuristicsAsDives::heuristic_runs() const { return heuristic_runs_; }

RunHeuristicsAsDives::HeuristicWrapper::HeuristicWrapper(
    Solver* solver, const std::vector<IntVar*>& vars,
    Solver::IntVarStrategy var_strategy,
    Solver::IntValueStrategy value_strategy, const std::string& heuristic_name,
    int heuristic_runs)
...
DefaultIntegerSearch::DefaultIntegerSearch(
    Solver* solver, const std::vector<IntVar*>& vars,
...
Decision* DefaultIntegerSearch::Next(Solver* solver) {
  CheckInit(solver);
...
void DefaultIntegerSearch::ClearLastDecision() {
  last_int_var_ = nullptr;
  last_int_value_ = 0;
  last_operation_ = FindVar::NONE;
}

void DefaultIntegerSearch::AppendMonitors(Solver* solver,
...
void DefaultIntegerSearch::Accept(ModelVisitor* visitor) const {
...
std::string DefaultIntegerSearch::DebugString() const {
...
std::string DefaultIntegerSearch::StatString() const {
...
void DefaultIntegerSearch::CheckInit(Solver* solver) {
  if (init_done_) {
    return;
  }
  if (parameters_.decision_builder == nullptr) {
...
Decision* DefaultIntegerSearch::ImpactNext(Solver* solver) {
  IntVar* var = nullptr;
  int64_t value = 0;
...
// DefaultPhaseStatString implementation
