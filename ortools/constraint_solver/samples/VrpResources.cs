...
public class VrpResources {
  ...
  static void PrintSolution(in DataModel data, in RoutingModel routing,
    ...
    for (int i = 0; i < data.VehicleNumber; ++i) {
      ...
      while (routing.IsEnd(index) == false) {
        ...
        Console.Write("{0} Time({1},{2}) -> ", manager.IndexToNode(index),
                      solution.Min(timeVar), solution.Max(timeVar));
        index = solution.Value(routing.NextVar(index));
      }
      var endTimeVar = timeDimension.CumulVar(index);
      Console.WriteLine("{0} Time({1},{2})", manager.IndexToNode(index),
                        solution.Min(endTimeVar), solution.Max(endTimeVar));
  ...
  public static void Main(String[] args) {
    ...
    RoutingIndexManager manager = new RoutingIndexManager(
        data.TimeMatrix.GetLength(0), data.VehicleNumber, data.Depot);
    // [END index_manager]
    ...
    solver.Add(solver.MakeCumulative(intervals, depot_usage, data.DepotCapacity,
                                     "depot"));
    // [END depot_capacity]
