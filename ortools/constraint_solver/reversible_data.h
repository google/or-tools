...
#ifndef ORTOOLS_CONSTRAINT_SOLVER_REVERSIBLE_DATA_H_
#define ORTOOLS_CONSTRAINT_SOLVER_REVERSIBLE_DATA_H_
...
class Rev {
 public:
  ...
  void SetValue(ReversibleEngine* engine, const T& val) {
    if (val != value_) {
      if (stamp_ < engine->stamp()) {
        engine->SaveValue(&value_);
        stamp_ = engine->stamp();
      }
      value_ = val;
    }
...
class NumericalRev : public Rev<T> {
 public:
  ...
  void Add(ReversibleEngine* engine, const T& to_add) {
    this->SetValue(engine, this->Value() + to_add);
  }

  void Incr(ReversibleEngine* engine) { Add(engine, 1); }

  void Decr(ReversibleEngine* engine) { Add(engine, -1); }
};
...
class RevArray {
 public:
  ...
  void SetValue(ReversibleEngine* engine, int index, const T& val) {
    DCHECK_LT(index, size_);
    if (val != values_[index]) {
      if (stamps_[index] < engine->stamp()) {
        engine->SaveValue(&values_[index]);
        stamps_[index] = engine->stamp();
      }
      values_[index] = val;
    }
  }
...
class NumericalRevArray : public RevArray<T> {
 public:
  ...
  void Add(ReversibleEngine* engine, int index, const T& to_add) {
    this->SetValue(engine, index, this->Value(index) + to_add);
  }

  void Incr(ReversibleEngine* engine, int index) { Add(engine, index, 1); }

  void Decr(ReversibleEngine* engine, int index) { Add(engine, index, -1); }
};

#ifndef SWIG
/// This class represent a reversible FIFO structure.
/// The main difference w.r.t a standard FIFO structure is that a Solver is
/// given as parameter to the modifiers such that the engine can store the
...
class SimpleRevFIFO {
 ...
 public:
  ...
  void Push(ReversibleEngine* engine, T val) {
    if (pos_.Value() == 0) {
      Chunk* const chunk = engine->UnsafeRevAlloc(new Chunk(chunks_));
      engine->SaveAndSetValue(reinterpret_cast<void**>(&chunks_),
                              reinterpret_cast<void*>(chunk));
      pos_.SetValue(engine, CHUNK_SIZE - 1);
    } else {
      pos_.Decr(engine);
    }
  ...
  void PushIfNotTop(ReversibleEngine* engine, T val) {
...
class RevIntSet {
 public:
  ...
  void Insert(ReversibleEngine* engine, const T& elt) {
    ...
    num_elements_.Incr(engine);
  }

  void Remove(ReversibleEngine* engine, const T& value_index) {
    num_elements_.Decr(engine);
    SwapTo(value_index, num_elements_.Value());
  }

  void Restore(ReversibleEngine* engine, const T& value_index) {
    SwapTo(value_index, num_elements_.Value());
    num_elements_.Incr(engine);
  }

  void Clear(ReversibleEngine* engine) { num_elements_.SetValue(engine, 0); }
...
class SmallRevBitSet {
 public:
  ...
  /// Sets the 'pos' bit.
  void SetToOne(ReversibleEngine* engine, int64_t pos);
  /// Erases the 'pos' bit.
  void SetToZero(ReversibleEngine* engine, int64_t pos);
...
class RevBitSet {
 public:
  ...
  /// Sets the 'index' bit.
  void SetToOne(ReversibleEngine* engine, int64_t index);
  /// Erases the 'index' bit.
  void SetToZero(ReversibleEngine* engine, int64_t index);
  ...
  /// Cleans all bits.
  void ClearAll(ReversibleEngine* engine);

  friend class RevBitMatrix;

 private:
  /// Save the offset's part of the bitset.
  void Save(ReversibleEngine* engine, int offset);
  const int64_t size_;
...
class RevBitMatrix : private RevBitSet {
 public:
  ...
  void SetToOne(ReversibleEngine* engine, int64_t row, int64_t column);
  /// Erases the 'column' bit in the 'row' row.
  void SetToZero(ReversibleEngine* engine, int64_t row, int64_t column);
...
class UnsortedNullableRevBitset {
 public:
  ...
  void Init(ReversibleEngine* engine, const std::vector<uint64_t>& mask);

  /// This method subtracts the mask from the active bitset. It returns true if
  /// the active bitset was changed in the process.
  bool RevSubtract(ReversibleEngine* engine, const std::vector<uint64_t>& mask);

  /// This method ANDs the mask with the active bitset. It returns true if
  /// the active bitset was changed in the process.
  bool RevAnd(ReversibleEngine* engine, const std::vector<uint64_t>& mask);
 ...
 private:
  void CleanUpActives(ReversibleEngine* engine);
...
class RevSwitch {
 public:
  ...
  void Switch(ReversibleEngine* engine) {
    engine->SaveAndSetValue(&value_, true);
  }
...
/// have a type T. As it relies on the engine for reversibility, these
...
class RevGrowingArray {
 public:
  ...
  void RevInsert(ReversibleEngine* engine, int64_t index, T value) {
    const int64_t block_index = ComputeBlockIndex(index);
    T* const block = GetOrCreateBlock(block_index);
    const int64_t residual = index - block_index * block_size_;
    engine->SaveAndSetValue(reinterpret_cast<C*>(&block[residual]),
...
class RevPartialSequence {
 public:
  ...
  void RankFirst(ReversibleEngine* engine, int elt) {
    DCHECK_LE(first_ranked_.Value(), last_ranked_.Value());
    SwapTo(elt, first_ranked_.Value());
    first_ranked_.Incr(engine);
  }

  void RankLast(ReversibleEngine* engine, int elt) {
    DCHECK_LE(first_ranked_.Value(), last_ranked_.Value());
    SwapTo(elt, last_ranked_.Value());
    last_ranked_.Decr(engine);
  }
...
#endif  // ORTOOLS_CONSTRAINT_SOLVER_REVERSIBLE_DATA_H_
