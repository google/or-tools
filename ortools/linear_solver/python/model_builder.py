...
def _add_enforced_linear_constraint_to_helper(
  ...
  if isinstance(bounded_expr, mbh.BoundedLinearExpression):
    ...
    helper.set_enforced_constraint_lower_bound(
        c.index, bounded_expr.lower_bound
    )
    helper.set_enforced_constraint_upper_bound(
        c.index, bounded_expr.upper_bound
    )
    if name is not None:
      helper.set_constraint_name(c.index, name)
    return c
...
class Model:
  ...
  * ```add_``` create new constraints and add them to the model.
  ...
  def _get_linear_constraints(
      self, constraints: Optional[pd.Index]
  ) -> pd.Index:
    ...
  ...
  def add_enforced(
    ...
    if isinstance(ct, mbh.BoundedLinearExpression):
      ...
      raise TypeError(
          f"Not supported: Model.add_enforced({type(ct).__name__!r}"
      )
