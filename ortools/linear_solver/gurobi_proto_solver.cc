...
inline absl::Status
GurobiCodeToUtilStatus(int error_code, const char *source_file, int source_line,
                       const char *statement, GRBenv *const env) {
  if (error_code == GRB_OK)
...
int AddIndicatorConstraint(const MPGeneralConstraintProto &gen_cst,
  ...
  if (cst.upper_bound() < std::numeric_limits<double>::infinity() &&
    ...
    return GRBaddgenconstrIndicator(
        gurobi_model, gen_cst.name().c_str(), ind_cst.var_index(),
        ind_cst.var_value(), cst.var_index_size(),
        cst.mutable_var_index()->mutable_data(),
        cst.mutable_coefficient()->mutable_data(), GRB_LESS_EQUAL,
        cst.upper_bound());
  }
...
GurobiSolveProto(const MPModelRequest &request, GRBenv *gurobi_env) {
  ...
  auto gurobi_model_deleter = absl::MakeCleanup([&]() {
    ...
    LOG_IF(DFATAL, error_code != GRB_OK) << "GRBfreemodel failed with error "
                                         << error_code << ": "
...
#define RETURN_IF_GUROBI_ERROR(x)                                              \
  ...
  if (request.solver_time_limit_seconds() > 0) {
    RETURN_IF_GUROBI_ERROR(
        GRBsetdblparam(GRBgetenv(gurobi_model), GRB_DBL_PAR_TIMELIMIT,
  ...
  bool has_integer_variables = false;
  {
    ...
    for (int v = 0; v < variable_size; ++v) {
      ...
      if (!variable.name().empty())
        varnames[v] = variable.name().c_str();
    }
...

  {
    ...
    for (const auto &gen_cst : model.general_constraint()) {
      ...
      case MPGeneralConstraintProto::kSosConstraint: {
        RETURN_IF_GUROBI_ERROR(
            AddSosConstraint(gen_cst.sos_constraint(), gurobi_model,
