...
///
/// This class can be used to represent such set efficiently as a
/// sorted and non-adjacent list of intervals. This is efficient as
/// long as the size of such list stays reasonable.
///
...
class Domain {
 public:
  ...
  /// Basic read-only std::vector<> wrapping to view a Domain as a
  /// sorted list of non-adjacent intervals. Note that we don't expose
...
/// When an interval is inserted, all intervals that overlap it or
/// that are even adjacent to it are merged into one. I.e. [0,14] and
/// [15,30] will be merged to [0,30].
///
...
class SortedDisjointIntervalList {
 public:
  ...
  /// [2, 5] and [7, 8] are not).
  ///
  /// Returns an iterator to the inserted interval (possibly merged
  /// with others).
  ///
  /// If start > end, it does LOG(DFATAL) and returns end() (no interval added).
  Iterator InsertInterval(int64 start, int64 end);

  /// If value is in an interval, increase its end by one, otherwise
  /// insert the interval [value, value]. In both cases, this returns
  /// an iterator to the new/modified interval (possibly merged with
  /// others) and fills newly_covered with the new value that was just
  /// added in the union of all the intervals.
  ///
