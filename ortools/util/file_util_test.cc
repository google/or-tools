...
class FileUtilsTest : public ::testing::Test {
 protected:
  static constexpr char kFileName[] = "/mmemfile/file";
  static constexpr char kEmptyFileName[] = "/mmemfile/empty";
  static constexpr char kUnknownFileName[] = "/mmemfile/does/not/exist";

  void SetUp() override {
    MutableMemFileRegister(kEmptyFileName);
    MutableMemFileRegister(kFileName);
  }

  void TearDown() override {
    MutableMemFileUnregister(kEmptyFileName);
    MutableMemFileUnregister(kFileName);
  }
};
...
TEST_F(ReadFileToStringTest, SmallTextFile) {
  ...
  ASSERT_OK(file::SetContents(kFileName, contents, file::Defaults()));

  EXPECT_THAT(ReadFileToString(kFileName), IsOkAndHolds(contents));
}

TEST_F(ReadFileToStringTest, EmptyFile) {
  EXPECT_THAT(ReadFileToString(kEmptyFileName), IsOkAndHolds(""));
}

TEST_F(ReadFileToStringTest, UnknownFile) {
  EXPECT_THAT(ReadFileToString(kUnknownFileName),
              StatusIs(util::error::NOT_FOUND));
}

TEST_F(ReadFileToStringTest, GzippedTextFile) {
  constexpr absl::string_view contents = "line 1\nline 2";
  RegisteredMutableMemFile registered(kFileName);
  std::string compressed;
  GzipString(contents, &compressed);
  ASSERT_OK(file::SetContents(kFileName, compressed, file::Defaults()));
...
TEST_F(ReadFileToProtoTest, FileError) {
  FileUtilTestProto2 proto;
  EXPECT_THAT(
      ReadFileToProto(kUnknownFileName, &proto),
      StatusIs(absl::StatusCode::kNotFound, HasSubstr(kUnknownFileName)));
}

TEST_F(ReadFileToProtoTest, OutputsFileNameInErrorStatus) {
  FileUtilTestProto2 proto;
  ASSERT_OK(file::SetContents(kFileName, "gloubi-boulga", file::Defaults()));
  EXPECT_THAT(
      ReadFileToProto(kFileName, &proto),
      StatusIs(absl::StatusCode::kInvalidArgument, HasSubstr(kFileName)));
}
...
TEST(ReadAndWriteProtoToFileTest, Gzipped) {
  ...
  EXPECT_LE(
      file::GetSize(gzipped_filename + ".gz", file::Defaults()).ValueOrDie(),
      0.8 * file::GetSize(filename, file::Defaults()).ValueOrDie());
  EXPECT_EQ(
      file::GetSize(gzipped_filename + ".gz", file::Defaults()).ValueOrDie(),
      file::GetSize(gzipped_filename2, file::Defaults()).ValueOrDie());
}
...
TEST_F(ReadFileToProtoStatusOrTest, Success) {
  ...
  ASSERT_OK(file::SetContents(kFileName, model_text, file::Defaults()));
  EXPECT_THAT(ReadFileToProto<FileUtilTestProto1>(kFileName),
              IsOkAndHolds(EqualsProto(model)));
}

TEST_F(ReadFileToProtoStatusOrTest, FileError) {
  EXPECT_THAT(
      ReadFileToProto<FileUtilTestProto1>(kUnknownFileName).status(),
      StatusIs(absl::StatusCode::kNotFound, HasSubstr(kUnknownFileName)));
}
