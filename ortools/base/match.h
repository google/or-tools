...
#ifndef OR_TOOLS_BASE_MATCH_H_
#define OR_TOOLS_BASE_MATCH_H_
...
inline bool StartsWith(string_view s, string_view x) {
  return s.size() >= x.size() && memcmp(s.data(), x.data(), x.size()) == 0;
}

// Returns whether s ends with x.
inline bool EndsWith(string_view s, string_view x) {
  return s.size() >= x.size() &&
         memcmp(s.data() + (s.size() - x.size()), x.data(), x.size()) == 0;
}
}  // namespace absl
#endif  // OR_TOOLS_BASE_MATCH_H_
