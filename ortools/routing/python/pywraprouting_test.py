...
class TestPyWrapRoutingModel(absltest.TestCase):
  ...
  def testSolve(self):
    ...
    self.assertEqual(
        enums_pb2.RoutingSearchStatus.ROUTING_OPTIMAL, model.status()
    )
  ...
  def testTransitCallback(self):
    ...
    self.assertEqual(
        enums_pb2.RoutingSearchStatus.ROUTING_SUCCESS, model.status()
    )
  ...
  def testFindErrorInRoutingSearchParameters(self):
    ...
    self.assertIn(
        "cross", pywraprouting.FindErrorInRoutingSearchParameters(params)
    )
