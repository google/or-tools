...
class ModelTest(absltest.TestCase):
  ...
  def testSolve(self):
    ...
    self.assertEqual(
        RoutingSearchStatus.ROUTING_OPTIMAL, routing_model.status()
    )
  ...
  def testTransitCallback(self):
    ...
    self.assertEqual(
        RoutingSearchStatus.ROUTING_SUCCESS, routing_model.status()
    )
  ...
  def testVectorDimensionTSP(self):
    ...
    while not routing_model.is_end(node):
      self.assertEqual(
          cumul, assignment.value(vector_dimension.cumul_var(node))
      )
      cumul += values[node]
      node = assignment.value(routing_model.next_var(node))
  ...
  def testMatrixDimensionTSP(self):
    ...
    while not routing_model.is_end(index):
      ...
      cumul += values[manager.index_to_node(index)][
