...
void KnapsackPropagatorForCuts::Init(const std::vector<double>& profits,
  ...
  for (int i = 0; i < number_of_items; ++i) {
    items_.emplace_back(
        MakeUnique<KnapsackItemForCuts>(i, weights[i], profits[i]));
  }
  capacity_ = capacity;
  current_profit_ = 0;
...
void KnapsackPropagatorForCuts::InitPropagator() {
  ...
  for (int i(0); i < items().size(); ++i) {
    sorted_items_.emplace_back(MakeUnique<KnapsackItemForCuts>(
...
double KnapsackSolverForCuts::Solve(TimeLimit* time_limit,
  ...
  *is_solution_optimal = true;
  ...
  auto root_node = MakeUnique<KnapsackSearchNodeForCuts>(nullptr, assignment);
...
bool KnapsackSolverForCuts::MakeNewNode(const KnapsackSearchNodeForCuts& node,
  ...
  auto relevant_node = MakeUnique<KnapsackSearchNodeForCuts>(&node, assignment);
