...
class SetCoverInvariant {
 public:
  ...
  enum class ConsistencyLevel {
    ...
    kCostAndCoverage,   // cost_, coverage_ and num_uncovered_elements_.
  ...
  // Returns a list of subsets (a `focus`) which is the union of all the subsets
  // that may be used to cover the uncovered elements.
  std::vector<SubsetIndex> ComputeUncoveredFocus() const;
 ...
 private:
  // Returns the number of uncovered elements in the given coverage vector.
  // Used in other functions in this class.
  BaseInt ComputeNumUncoveredElements(const ElementToIntVector& coverage) const;

  // Returns the cost, the coverage vector and the number of uncovered
  // elements for the given choices.
  // Temporarily uses |E| BaseInts.
  // Does not update the timestamp, because it is const and private.
  std::tuple<Cost, ElementToIntVector, BaseInt> ComputeCostAndCoverage(
