...
absl::StatusOr<SolveResultProto> Solver::NonIncrementalSolve(
  ...
  ABSL_ASSIGN_OR_RETURN(std::unique_ptr<Solver> solver,
...
absl::StatusOr<std::unique_ptr<Solver>> Solver::New(
  ...
  ABSL_RETURN_IF_ERROR(internal::ValidateInitArgs(arguments, solver_type));
  ABSL_ASSIGN_OR_RETURN(ModelSummary summary, ValidateModel(model));
...
absl::StatusOr<SolveResultProto> Solver::Solve(const SolveArgs& arguments) {
  ABSL_ASSIGN_OR_RETURN(const auto guard, ConcurrentCallsGuard::TryAcquire(
  ...
  ABSL_RETURN_IF_ERROR(ValidateSolveParameters(arguments.parameters))
      << "invalid parameters";
  ABSL_RETURN_IF_ERROR(
      ValidateModelSolveParameters(arguments.model_parameters, model_summary_))
      << "invalid model_parameters";

  ABSL_RETURN_IF_ERROR(ValidateCallbackRegistration(
  ...
  ABSL_ASSIGN_OR_RETURN(
      ...
      underlying_solver_->Solve(
          arguments.parameters, arguments.model_parameters,
          arguments.message_callback, arguments.callback_registration, cb,
...
absl::StatusOr<bool> Solver::Update(const ModelUpdateProto model_update) {
  ...
  ABSL_RETURN_IF_ERROR(ValidateModelUpdate(model_update, model_summary_));
...
Solver::ComputeInfeasibleSubsystem(
  ...
  ABSL_ASSIGN_OR_RETURN(ComputeInfeasibleSubsystemResultProto result,
