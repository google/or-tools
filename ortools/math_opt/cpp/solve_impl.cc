...
absl::StatusOr<SolveResult> CallSolve(BaseSolver& solver,
  ...
  OR_RETURN_IF_ERROR(arguments.CheckModelStorage(expected_storage));
  ...
  if (arguments.callback != nullptr) {
    cb = [&](const CallbackDataProto& callback_data_proto) {
      ...
      if (const absl::Status status =
          ...
          !status.ok()) {
        // Note that we use ortools::StatusBuilder() here as util::Annotate() is
        // not available in open-source code.
        ortools::StatusBuilder builder(status);
  ...
  OR_ASSIGN_OR_RETURN(ModelSolveParametersProto model_parameters,
...
IncrementalSolverImpl::New(
  ...
  OR_ASSIGN_OR_RETURN(ModelProto model_proto,
...
absl::StatusOr<UpdateResult> IncrementalSolverImpl::Update() {
  ...
  OR_ASSIGN_OR_RETURN(std::optional<ModelUpdateProto> model_update,
  ...
  OR_ASSIGN_OR_RETURN3(const bool did_update,
                       ...
                       _ << "update failed");
  OR_RETURN_IF_ERROR(update_tracker_->AdvanceCheckpoint());
