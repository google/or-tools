...
class ModelStorageTest(compare_proto.MathOptProtoAssertions, parameterized.TestCase):
    ...
    def test_update_var_integer_same_value(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
        tracker.advance_checkpoint()
    ...
    def test_update_lin_con_lb_same_value(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        c = storage.add_linear_constraint(-1.0, 2.5, "c")
        tracker.advance_checkpoint()
    ...
    def test_update_lin_con_ub_same_value(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        c = storage.add_linear_constraint(-1.0, 2.5, "c")
        tracker.advance_checkpoint()
    ...
    def test_update_lin_con_then_delete(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        c = storage.add_linear_constraint(-1.0, 2.5, "c")
        tracker.advance_checkpoint()
    ...
    def test_delete_var_before_first_update(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        tracker.advance_checkpoint()
        x = storage.add_variable(-1.0, 2.5, True, "x")
    ...
    def test_update_objective_direction(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
    ...
    def test_update_objective_offset_same(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        storage.set_objective_offset(0.0)
        self.assertIsNone(tracker.export_update())

    def test_objective_update_existing_zero(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
        tracker.advance_checkpoint()
    ...
    def test_objective_update_new(self, storage_class: _StorageClass) -> None:
        ...
        self.assert_protos_equiv(
            _ModelUpdateProto(
                ...
                objective_updates=_ObjectiveUpdatesProto(
                    linear_coefficients=_SparseDoubleVectorProto(ids=[0], values=[4.0])
                ),
            ),
            tracker.export_update(),
        )

    def test_objective_update_new_zero(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
        storage.set_linear_objective_coefficient(x, 4.0)
    ...
    def test_quadratic_objective_update_new(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
    ...
    def test_lin_con_mat_update_new_var(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        c = storage.add_linear_constraint(-1.0, 2.5, "c")
        tracker.advance_checkpoint()
    ...
    def test_lin_con_mat_update_new_lin_con(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
        tracker.advance_checkpoint()
    ...
    def test_lin_con_mat_update_new_both(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
        c = storage.add_linear_constraint(-1.0, 2.5, "c")
    ...
    def test_lin_con_mat_update_new_zero(self, storage_class: _StorageClass) -> None:
        storage = storage_class("test_model")
        tracker = storage.add_update_tracker()
        x = storage.add_variable(-1.0, 2.5, True, "x")
        tracker.advance_checkpoint()
    ...
    def test_multiple_update_tracker(self, storage_class: _StorageClass) -> None:
        ...
        self.assert_protos_equiv(
            _ModelUpdateProto(
                variable_updates=_VariableUpdatesProto(
                    upper_bounds=_SparseDoubleVectorProto(ids=[0, 1], values=[7.0, 3.0])
                )
            ),
