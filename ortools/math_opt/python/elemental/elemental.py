...
class Elemental(Protocol):
  ...
  def is_attr_non_default(
      self, attr: enums.AnyAttr, key: Sequence[int]
  ) -> bool:
  ...
  def get_attr_non_defaults(
      self, attr: enums.AnyAttr
  ) -> np.typing.NDArray[np.int64]:
  ...
  def apply_update(
      self, update_proto: model_update_pb2.ModelUpdateProto
  ) -> None:
