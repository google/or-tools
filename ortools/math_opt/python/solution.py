...
def optional_solution_status_to_proto(
  ...
  return (
      solution_pb2.SOLUTION_STATUS_UNSPECIFIED
      if status is None
      else status.value
  )
...
class DualSolution:
  ...
  dual_values: Dict[linear_constraints.LinearConstraint, float] = (
      dataclasses.field(default_factory=dict)
  )
  quadratic_dual_values: Dict[
      quadratic_constraints.QuadraticConstraint, float
  ] = dataclasses.field(default_factory=dict)
...
class Basis:
  ...
  def to_proto(self) -> solution_pb2.BasisProto:
    ...
    return solution_pb2.BasisProto(
        variable_status=_to_sparse_basis_status_vector_proto(
...
def parse_basis(
  ...
  for index, vid in enumerate(proto.variable_status.ids):
    ...
    result.variable_status[mod.get_variable(vid, validate=validate)] = (
  ...
  for index, cid in enumerate(proto.constraint_status.ids):
    ...
    result.constraint_status[
        mod.get_linear_constraint(cid, validate=validate)
    ] = BasisStatus(status_proto)
