...
class Variable(LinearBase, from_model.FromModel):
  ...
  def integer(self) -> bool:
    return self._elemental.get_attr(
        enums.BoolAttr1.VARIABLE_INTEGER, (self._id,)
    )

  @integer.setter
  def integer(self, value: bool) -> None:
    self._elemental.set_attr(
        enums.BoolAttr1.VARIABLE_INTEGER, (self._id,), value
    )

  @property
  def name(self) -> str:
    return self._elemental.get_element_name(
        enums.ElementType.VARIABLE, self._id
    )
...
class QuadraticExpression(QuadraticBase):
  ...
  def __init__(self, other: QuadraticTypes) -> None:
    ...
    if isinstance(other, (int, float)):
      self._offset = float(other)
      self._linear_terms: Mapping[Variable, float] = (
  ...
  def evaluate(self, variable_values: Mapping[Variable, float]) -> float:
    ...
    for key, coef in sorted(
      ...
      result += (
          coef
          * variable_values[key.first_var]
          * variable_values[key.second_var]
      )
    return result
  ...
  def __repr__(self):
    ...
    result += ", ".join(
        f"{var!r}: {coefficient}"
        for var, coefficient in self._linear_terms.items()
    )
    result += "}, {"
    result += ", ".join(
        f"{key!r}: {coefficient}"
...
class LinearLinearProduct(QuadraticBase):
  ...
  def __init__(
      self, first_linear: LinearBase, second_linear: LinearBase
  ) -> None:
