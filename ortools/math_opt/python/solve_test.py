...
def _list_is_near(
    v1: List[float], v2: List[float], tolerance: float = 1e-5
) -> bool:
  if len(v1) != len(v2):
    return False
...
class SolveTest(absltest.TestCase):
  ...
  def test_solve_error(self) -> None:
    ...
    with self.assertRaisesRegex(
        ValueError, "variables.*lower_bound > upper_bound"
    ):
  ...
  def test_lp_solve(self) -> None:
    ...
    self.assertAlmostEqual(
        2.0, res.solutions[0].primal_solution.objective_value
    )
    ...
    self.assertTrue(
        _list_is_near(expected1, dual_vec)
        or _list_is_near(expected2, dual_vec),
  ...
  def test_filters(self) -> None:
    ...
    res = solve.solve(
        mod, parameters.SolverType.GLOP, model_params=model_params
    )
    self.assertEqual(
        res.termination.reason,
        result.TerminationReason.OPTIMAL,
  ...
  def test_incremental_lp(self) -> None:
    ...
    self.assertAlmostEqual(
        6.0, res2.solutions[0].primal_solution.objective_value
    )
    self._assert_dict_almost_equal(
  ...
  def test_incremental_mip(self) -> None:
    ...
    assert res.solutions[0].primal_solution is not None
    self.assertAlmostEqual(
        3.0, res.solutions[0].primal_solution.objective_value
    )
    self._assert_dict_almost_equal(
    ...
    self.assertAlmostEqual(
        5.0, res2.solutions[0].primal_solution.objective_value
    )
    self._assert_dict_almost_equal(
  ...
  def test_incremental_solver_context_manager(self) -> None:
    opt_model = model.Model()
    start_num_solver = core_solver.debug_num_solver()
    with solve.IncrementalSolver(
        opt_model, parameters.SolverType.GLOP
    ) as solver:
      self.assertEqual(core_solver.debug_num_solver(), start_num_solver + 1)
      res1 = solver.solve()
      self.assertEqual(
