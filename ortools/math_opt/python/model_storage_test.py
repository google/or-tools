...
class ModelStorageTest(
  ...
  def test_delete_variable_get_other(
      self, storage_class: _StorageClass
  ) -> None:
    storage = storage_class()
    v1 = storage.add_variable(-1.0, 2.5, True, "x")
  ...
  def test_delete_variable_name_error(
      self, storage_class: _StorageClass
  ) -> None:
    self._deleted_variable_invoke_lookup(
  ...
  def test_delete_variable_set_lb_error(
      self, storage_class: _StorageClass
  ) -> None:
    storage = storage_class()
    x = storage.add_variable(-1.0, 2.5, True, "x")
  ...
  def test_clear_quadratic_objective_coefficient(
    ...
    self.assertEmpty(
        list(storage.get_quadratic_objective_adjacent_variables(y))
    )
    self.assertEmpty(
        list(storage.get_quadratic_objective_adjacent_variables(z))
    )
  ...
  def test_constraint_matrix(self, storage_class: _StorageClass) -> None:
    ...
    self.assertCountEqual(
        [x, y], storage.get_variables_for_linear_constraint(d)
    )

    self.assertCountEqual([d], storage.get_linear_constraints_with_variable(x))
    self.assertCountEqual(
        [c, d], storage.get_linear_constraints_with_variable(y)
    )
  ...
  def test_proto_export_with_deletes(
      self, storage_class: _StorageClass
  ) -> None:
    storage = storage_class("test_model")
    x = storage.add_variable(-1.0, 2.5, True, "x")
  ...
  def test_nans_pass_through(self, storage_class: _StorageClass) -> None:
    ...
    self.assertCountEqual(
        [x, y], storage.get_variables_for_linear_constraint(c)
    )
