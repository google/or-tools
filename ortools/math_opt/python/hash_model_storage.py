...
class _UpdateTracker(model_storage.StorageUpdateTracker):
  ...
  def export_update(self) -> Optional[model_update_pb2.ModelUpdateProto]:
    ...
    result.deleted_linear_constraint_ids[:] = sorted(
        self.linear_constraint_deletes
    )
    # Variable updates
    _set_sparse_double_vector(
        sorted(
            (vid, self.model.get_variable_lb(vid)) for vid in self.variable_lbs
        ),
        result.variable_updates.lower_bounds,
    )
    _set_sparse_double_vector(
        sorted(
            (vid, self.model.get_variable_ub(vid)) for vid in self.variable_ubs
        ),
      ...
      if self.model.variable_exists(new_var):
        for lin_con in self.model.get_linear_constraints_with_variable(new_var):
          matrix_updates.append((
              lin_con,
              new_var,
              self.model.get_linear_constraint_coefficient(lin_con, new_var),
          ))
        ...
        for var in self.model.get_variables_for_linear_constraint(new_lin_con):
          ...
          if var < self.variables_checkpoint:
            matrix_updates.append((
                new_lin_con,
                var,
                self.model.get_linear_constraint_coefficient(new_lin_con, var),
            ))
    matrix_updates.sort()
    if matrix_updates:
...
class HashModelStorage(model_storage.ModelStorage):
  ...
  def add_variable(
      self, lb: float, ub: float, is_integer: bool, name: str
  ) -> int:
      ...
      if variable_id < watcher.variables_checkpoint:
        ...
        for lin_con_id in variable.linear_constraint_nonzeros:
          if lin_con_id < watcher.linear_constraints_checkpoint:
            watcher.linear_constraint_matrix.discard((lin_con_id, variable_id))
    # Then update self.
    for lin_con_id in variable.linear_constraint_nonzeros:
  ...
  def set_linear_constraint_lb(
      self, linear_constraint_id: int, lb: float
  ) -> None:
    self._check_linear_constraint_id(linear_constraint_id)
  ...
  def set_linear_constraint_ub(
      self, linear_constraint_id: int, ub: float
  ) -> None:
    self._check_linear_constraint_id(linear_constraint_id)
  ...
  def set_linear_constraint_coefficient(
    ...
    if value == 0.0:
      ...
      self._linear_constraint_matrix[(linear_constraint_id, variable_id)] = (
          value
      )
  ...
  def get_linear_constraints_with_variable(
  ...
  def set_linear_objective_coefficient(
      self, variable_id: int, value: float
  ) -> None:
    self._check_variable_id(variable_id)
  ...
  def export_model(self) -> model_pb2.ModelProto:
    ...
    if self.linear_objective_coefficient:
      obj_ids, obj_coefs = zip(
          *sorted(self.linear_objective_coefficient.items())
      )
