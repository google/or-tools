...
class ModelSubset:
  ...
  def to_proto(self) -> infeasible_subsystem_pb2.ModelSubsetProto:
    ...
    return infeasible_subsystem_pb2.ModelSubsetProto(
        ...
        variable_integrality=sorted(
            var.id for var in self.variable_integrality
        ),
        linear_constraints={
...
def parse_model_subset(
  ...
  return ModelSubset(
      ...
      variable_integrality=frozenset(
          mod.get_variable(var_id)
          for var_id in model_subset.variable_integrality
      ),
      linear_constraints={
...
def parse_compute_infeasible_subsystem_result(
  ...
  return ComputeInfeasibleSubsystemResult(
      feasibility=result.FeasibilityStatus(
          infeasible_system_result.feasibility
      ),
