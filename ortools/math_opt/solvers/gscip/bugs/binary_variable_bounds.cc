...
// A simple example where changing the bounds on a binary variable in (g)Scip
// can lead to unexpected behaviors: either incorrect bounds, or an internal
// CHECK-fail in debug mode.
#include <memory>
#include <utility>
...
int main(int argc, char* argv[]) {
  ...
  const auto x_or = gscip->AddVariable(
      0.0, 1.0, 0.0, operations_research::GScipVarType::kBinary, "x");
  QCHECK_OK(x_or);
