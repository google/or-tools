...
TEST(GScipTest, BadConstraint) {
  ...
  range.coefficients = {1.0, 1.0};
  EXPECT_THAT(
      gscip->AddLinearConstraint(range, "c1"),
      StatusIs(INVALID_ARGUMENT, HasSubstr("Error adding constraint: c1")));
}
...
TEST(GScipTest, NoVariablesSOS1) {
  ...
  sos1_data.variables = {};
  EXPECT_THAT(
      gscip->AddSOS1Constraint(sos1_data, "c1"),
      StatusIs(INVALID_ARGUMENT, HasSubstr("Error adding SOS constraint: c1")));
}
...
TEST(GScipTest, VariablesMatchWeightsSOS2) {
  ...
  EXPECT_THAT(
      gscip->AddSOS2Constraint(sos2_data, "c1"),
      StatusIs(INVALID_ARGUMENT, HasSubstr("Error adding SOS constraint: c1")));
}
