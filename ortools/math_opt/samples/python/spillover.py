...
def _random_spillover_problem(
    num_machines: int, num_vms: int, fungibility: int
) -> SpilloverProblem:
    ...
    for _ in range(num_vms):
        ...
        vm_demands.append(
            VmDemand(
                compatible_machines=tuple(meet_demand),
                vm_quantity=random.randint(50, 100),
            )
        )
...
def main(argv: Sequence[str]) -> None:
    ...
    if _TEST_DATA.value:
        ...
        problem = _random_spillover_problem(
            _MACHINE_TYPES.value, _VM_TYPES.value, _FUNGIBILITY.value
        )
