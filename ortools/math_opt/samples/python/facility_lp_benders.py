...
_NUM_FACILITIES = flags.DEFINE_integer(
    "num_facilities", 3000, "Number of facilities."
)
_NUM_LOCATIONS = flags.DEFINE_integer(
    "num_locations", 50, "Number of locations."
)
_EDGE_PROBABILITY = flags.DEFINE_float(
    "edge_probability", 0.99, "Edge probability."
)
...
_FACILITY_COST = flags.DEFINE_float(
    "facility_cost", 100, "Facility capacity cost."
)
...
def full_problem(
  ...
  objective_for_edges = sum(
      instance.network.edge_costs[edge] * x[edge]
  ...
  if result.termination.reason != mathopt.TerminationReason.OPTIMAL:
    raise RuntimeError(
        f"failed to find an optimal solution: {result.termination}"
    )
...
def benders(
  ...
  while True:
    ...
    if (
        first_stage_result.termination.reason
        != mathopt.TerminationReason.OPTIMAL
    ):
      raise RuntimeError(
