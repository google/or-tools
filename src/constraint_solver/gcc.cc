 ...
 * on the restricted gcc constraint when bound on
 ...
 public:
  ...
  void InitialPropagate() {
    ...
    if (last_level_ != (current_level_.Value() - 1)) {
      ...
      has_changed = false;
      for(Index i(0); i < size_; i++) {
        const int64 dl = intervals_[i].min_value;
        const int64 du = intervals_[i].max_value;
        intervals_[i].min_value = variables_[i]->Min();
        intervals_[i].max_value = variables_[i]->Max();
        if (dl != intervals_[i].min_value || du != intervals_[i].max_value) {
          has_changed = true;
        }
      }
    }
  ...
  bool FilterLowerMin() {
    ...
    for (int64 i = 0; i < size_; i++) {
      ...
      if (z != x+1) {
        ...
        v = potential_stable_sets_[w = PathMax(potential_stable_sets_, x + 1)];
      ...
      if (diffs_[z] <= lower_sum_.Sum(bounds_[y], bounds_[z] - 1)) {
        ...
        if (diffs_[z] == lower_sum_.Sum(bounds_[y], bounds_[z] - 1)) {
          ...
          if (hall_[y] > y)
            // Equintervals_alent to PathMax since the path is fully
            // compressed
            y = hall_[y];
          PathSet(&hall_,
                  hall_[y],
...
Constraint* NewGCC(Solver* const solver,
                   const std::vector<IntVar*>& vars,
                   bool propagate_value,
                   int64 first_domain_value,
                   int64 last_domain_value,
                   const std::vector<int64>& min_occurrences,
                   const std::vector<int64>& max_occurrences) {
  return solver->RevAlloc(new GccConstraint(solver,
                                            vars,
                                            propagate_value,
                                            first_domain_value,
                                            last_domain_value,
