  ...
  uint##size BitCountRange##size(                                       \
    ...
    if (end - start > FLAGS_bitset_small_bitset_count) {                \
      ...
      if (offset_end == offset_start) {                                 \
        ...
        bit_count += BitCount##size(bits[offset_start]                  \
                                    & IntervalUp##size(pos_start));     \
        for (int offset = offset_start + 1;                             \
             offset < offset_end;                                       \
             ++offset) {                                                \
          bit_count += BitCount##size(bits[offset]);                    \
        }                                                               \
        bit_count += BitCount##size(bits[offset_end]                    \
                                    & IntervalDown##size(pos_end));     \
        return bit_count;                                               \
      }                                                                 \
    } else {                                                            \
      uint##size bit_count = zero;                                      \
...
#define IS_EMPTY_RANGE(size)                                            \
  bool IsEmptyRange##size(const uint##size* const bits,                 \
...
#define LEAST_SIGNIFCANT_BIT_POSITION(size)                             \
  int##size LeastSignificantBitPosition##size(                          \
    ...
    if (offset_start == offset_end) {                                   \
      ...
      const uint##size start_mask = bits[offset_start]                  \
          & IntervalUp##size(pos_start);                                \
      if (start_mask) {                                                 \
        return BitShift##size(offset_start)                             \
        ...
        const int pos_end = BitPos##size(end);                          \
        const uint##size active_range = bits[offset_end]                \
            & IntervalDown##size(pos_end);                              \
        if (active_range) {                                             \
