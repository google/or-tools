...
int FindSegmentIndex(const std::vector<PiecewiseSegment>& segments, int64 x) {
  ...
  std::vector<PiecewiseSegment>::const_iterator position =
    std::upper_bound(segments.begin(), segments.end(), x,
...
PiecewiseSegment::PiecewiseSegment(int64 point_x, int64 point_y,  int64 slope,
  ...
  intersection_y_ = reference_x_ < 0 ? SafeValuePostReference(0)
                                     : SafeValuePreReference(0);
}
...
int64 PiecewiseSegment::SafeValuePostReference(int64 x) const {
  ...
  if (slope_ == 0) {
    ...
    if (reference_y_ == 0) {
      ...
      if (span_y >= opp_reference_y) {
        return span_y - opp_reference_y > kint64max ?
               kint64max : static_cast<int64>(span_y - opp_reference_y);
      } else {
        return opp_reference_y - span_y > static_cast<uint64>(kint64max) + 1 ?
               kint64min : -static_cast<int64>(opp_reference_y - span_y);
      }
    }
  } else {
    ...
    const uint64 span_y = UnsignedCapProd(span_x, -slope_);
    if (reference_y_ == 0) {
      return span_y > kint64max ? kint64min
                                : -static_cast<int64>(span_y);
    } else if (reference_y_ < 0) {
      const uint64 opp_reference_y = static_cast<uint64>(-reference_y_);
      const uint64 opp_unsigned_sum = UnsignedCapAdd(opp_reference_y,
                                                     span_y);
      return opp_unsigned_sum > kint64max
               ? kint64min : -static_cast<int64>(opp_unsigned_sum);
    } else {
      if (reference_y_ >= span_y) {
        return reference_y_ - span_y > kint64max ?
               kint64max : static_cast<int64>(reference_y_ - span_y);
      } else {
        return span_y - reference_y_ > static_cast<uint64>(kint64max) + 1 ?
               kint64min : -static_cast<int64>(span_y - reference_y_);
      }
    }
  }
}
...
PiecewiseLinearFunction::PiecewiseLinearFunction(
  ...
  for (int i = 0; i < segments->size() - 1; ++i) {
    if (segments->at(i).end_x() > segments->at(i+1).start_x()) {
      LOG(FATAL) << "Overlapping segments: "
                 << segments->at(i).DebugString() << " & "
...
PiecewiseLinearFunction* PiecewiseLinearFunction::CreatePiecewiseLinearFunction(
  ...
  for (int i = 0; i < points_x.size(); ++i) {
    segments.push_back(PiecewiseSegment(points_x[i], points_y[i],
                                        slopes[i], other_points_x[i]));
  }
...
PiecewiseLinearFunction* PiecewiseLinearFunction::CreateFullDomainFunction(
  ...
  std::vector<PiecewiseSegment> segments;
  PiecewiseSegment segment = PiecewiseSegment(points_x[0], level,
                                              slopes[0], kint64min);
  segments.push_back(segment);
  level = segment.Value(points_x[0]);
  for (int i = 1; i < points_x.size(); ++i) {
    PiecewiseSegment segment = PiecewiseSegment(points_x[i-1], level,
                                                slopes[i], points_x[i]);
    segments.push_back(segment);
    level = segment.Value(points_x[i]);
  }
  segments.push_back(PiecewiseSegment(points_x.back(), level,
                                      slopes.back(), kint64max));
...
PiecewiseLinearFunction* PiecewiseLinearFunction::CreateEarlyTardyFunction(
  ...
  segments.push_back(PiecewiseSegment(reference, 0, -earliness_slope,
                                      kint64min));
  segments.push_back(PiecewiseSegment(reference, 0, tardiness_slope,
                                      kint64max));
...
PiecewiseLinearFunction*
  ...
  CHECK_GE(earliness_slope, 0);
  CHECK_GE(tardiness_slope, 0);
  segments.push_back(PiecewiseSegment(early_slack, 0,
                                      -earliness_slope, kint64min));
  segments.push_back(PiecewiseSegment(early_slack, 0,
                                      0, late_slack));
  segments.push_back(PiecewiseSegment(late_slack, 0,
                                      tardiness_slope, kint64max));
...
int64 PiecewiseLinearFunction::GetMaximum() const {
  return GetMaximum(segments_.front().start_x(),
                    segments_.back().end_x());
}

int64 PiecewiseLinearFunction::GetMinimum() const {
  return GetMinimum(segments_.front().start_x(),
                    segments_.back().end_x());
}
...
void PiecewiseLinearFunction::Operation(
  ...
  for (int64 start_x : start_x_points) {
    ...
    if (own_index >= 0 && other_index >= 0) {
      ...
      const int64 end_y = operation->Run(own_segment.Value(end_x),
                                         other_segment.Value(end_x));
      const int64 slope = operation->Run(own_segment.slope(),
                                         other_segment.slope());
